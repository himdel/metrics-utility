...
- **Evolution**: `metrics_utility/library/` created as a stub shared layer with collectors, dataframes, extractors, packaging, storage, reports and helpers (#218) -> filled in for real: five storage backends (#246), CLI collectors refactored into `controller/`+`others/` with env-var-free interfaces (#248), advisory lock extracted (#259), dataframe classes moved in with a loading/transform split (#237), `copy_table` switched to returning DataFrames (#327), then `dashboard/` (#341) and `service/` (#357) collector subpackages -> once metrics-service turned out to be the only external consumer, #540 pruned the library to what m-s imports: the reporting pipeline (dataframes, reports, extractors, package, instants, utils, csv_utils, debug), the non-Segment storage backends, `dashboard/filter_options` and the `workers/` examples were deleted, candlepin moved out to `metrics_utility/candlepin/`, and every collector stayed -> the prune exposed drift the duplication had hidden: the reDOS-safe FQCN regex from #342 had only ever been applied to the library copy, so deleting that copy silently reverted the fix and it had to be backported to the live CLI copy (#556) -> the collector surface then grew again with a fifth `awx/` subpackage of 15 ported AWX analytics collectors (#573).
- **PRs**: 9+ (#218, #246, #248, #237, #341, #357, #540, #556, #573)
...
### Indirect managed nodes: prototype to feature-gated group to collection-grouped daily to GA default-on, then defaults reverted
...
- **Evolution**: Collector added to METRICS_COLLECTION_GROUP (#274) -> integrated into daily anonymization pipeline (#296) -> extracted into own INDIRECT_NODE_COLLECTION_GROUP with INDIRECT_NODE_COLLECTION feature flag, default off (#301) -> fix: task function was never registered, silently never ran (#315) -> m-u rollup added with host_remote_id dedup (#446, #453, #457) -> moved hourly->snapshot on service side (#331) -> briefly converted to snapshot/until_slicing on library side (m-u #464) -> converted to daily slicing by job.finished with collection grouping, version bumped to 2.0 (m-u #475) -> moved snapshot->daily on service side with per-collector DB routing (#332). The rollup now groups by (org, collection_name) using events JSON parsing, and base() strips PII by removing host_names and org_name. -> dead code removal: by_organizations and by_collections computed in prepare()/merge() but never read by base() (#479) -> module-level breakdown added as parallel module_groups structure (#482) -> collection_name/module_name renamed to collection/module for Segment compatibility (#484) -> **GA default-on at AAP 2.7 (ANSTRAT-1628)**: coordinated cross-repo default flips -- m-u default-includes `main_indirectmanagednodeaudit` in `METRICS_UTILITY_OPTIONAL_COLLECTORS` (#560), default-enables the `indirectly_managed_nodes` report sheet and defaults `METRICS_UTILITY_REPORT_TYPE` to CCSPv2 (#559), and folds indirect nodes into the 'Usage by collections' report sheet (#558); m-s flips the `INDIRECT_NODE_COLLECTION` feature flag from opt-in `False` to `True` (#413). Disable paths (env var / installer / dynamic setting) preserved on both sides. -> **the m-u half of the GA flip was then fully reverted** over four days in a revert-of-a-revert chain (#579 -> #581 -> #582 + #583, see the dedicated arc below); m-u is back to opt-in defaults while m-s's `INDIRECT_NODE_COLLECTION=True` (#413) and m-u's Usage-by-Collections feature work (#558) both stand.
- **PRs**: 22+ (m-s #274, #296, #301, #315, #331, #332, #413; m-u #446, #453, #457, #464, #475, #479, #482, #484, #558, #559, #560, #579, #581, #582, #583)
...
- **Files**: ci_cd.md
- **Evolution**: the cross-repo OpenAPI spec-sync workflow added in #196 never ran. #394 made the generator work headless (correct `metrics_service.settings` module, `METRICS_SERVICE_` Dynaconf env vars, per-db SQLite so no Postgres, flat `metrics.json` output, versioned-branch targeting) and split it into generate + sync jobs -> #396 aligned it to the eng handbook drift-detection standard (artifact `openapi-schema`, `component-spec/` path, `diff -q` instead of `git diff`, `OPENAPI_SPEC_SYNC_TOKEN` secret, short-SHA branches, handbook PR template) and fixed a critical shell-injection by routing all `github.*` context through step `env` vars -> #397 added GPG signing with the shared `aap-api-bot` key for verified downstream commits (matching the Controller pattern) -> #403 reworked it to sync per stable branch (one PR per target branch via force-push + `gh pr list` reuse) with a new `OPENAPI_SPEC_PR_REVIEW_TEAM` secret.
- **PRs**: 4 (#394, #396, #397, #403), following broken #196

### Dependency pins loosened to compatible-release ranges (cross-repo)
- **Files**: dependencies_and_packaging.md
- **Evolution**: exact `==` runtime pins (original) -> m-u relaxed the setuptools build-tool pin `==80.9.0` -> `>=80.9.0` after it collided with Django's `setuptools>=83` build-system requirement in hermetic builds (#550) -> m-u loosened all shared runtime deps to compatible-release `~=major.minor` ranges so exact pins stop fighting the parent's combined `uv pip compile` when m-u is vendored under m-s (#552) -> m-s applied the same loosening (django/urllib3/pyjwt `==` -> `~=`) plus a Dependabot semver-major ignore rule (#406).
- **PRs**: 3 (m-u #550, #552; m-s #406)

### Indirect node GA defaults: land, revert, un-revert, revert again (net zero)
- **Files**: settings_and_configuration.md, metrics_utility.md
- **Evolution**: Three PRs landed the AAP-87148/87149/87150 GA work in m-u -- #558 (c00f979) folded indirect nodes into the `Usage by Collections` report sheet (AAP-89203, pure feature), #559 (902ce04) added `get_report_type()`/`get_optional_ccsp_report_sheets()` to `base/utils.py`, defaulted `METRICS_UTILITY_REPORT_TYPE` to `CCSPv2`, default-enabled `indirectly_managed_nodes` (both report types) and `infrastructure_summary` (CCSPv2 only), and added the CCSP warn-and-skip path, and #560 (76aadfd) added `main_indirectmanagednodeaudit` to the `METRICS_UTILITY_OPTIONAL_COLLECTORS` default while de-duplicating that default into `get_optional_collectors()`. Then: #579 (67220f6) hand-wrote a revert of #559 + #560 to "restore opt-in defaults", explicitly preserving #558 and proving it with a `git diff --exit-code devel...HEAD` check on the three report files -> #581 (c923cbd) reverted #579 wholesale, putting the defaults back -> the same day, #582 (3366d2e) reverted #559 as a clean `git revert` and #583 (a6ba967) reverted #560 the same way. **Net effect at HEAD: the defaults are exactly as they were before #559/#560.** `METRICS_UTILITY_REPORT_TYPE` is required again (`MissingRequiredEnvVar` when unset), `get_optional_collectors()` defaults to `'main_jobevent'` alone, `get_report_type()` and `get_optional_ccsp_report_sheets()` no longer exist, the sheet-list default is hardcoded separately in `build_report.py` and `validation.py` again, and `indirectly_managed_nodes`/`infrastructure_summary` are opt-in. Surviving the chain: #558's Usage-by-Collections feature (untouched throughout) and, re-landed standalone by #584 (574840b), the `VALID_SHEETS['CCSP']` entry plus the `ReportCCSP` warn-and-skip for an explicitly-requested `infrastructure_summary`. All CCSP/CCSPv2 snapshot `report.xlsx` fixtures made the round trip with the code.
- **PRs**: 6 (#559, #560 landed; #579, #581, #582, #583 the revert chain; #584 re-landed the separable half) -- the hand-written revert (#579) was the one that got reverted; the two clean `git revert`s (#582, #583) stuck
- **Lesson**: A hand-written "restore the old defaults" revert is harder to trust than two mechanical `git revert`s of the original PRs, and it got un-reverted before the mechanical ones landed. Also: #559 bundled a policy flip (default-on) with a refactor (de-duplicating the default sheet list) and a graceful-degradation feature (warn-and-skip) -- reverting the policy killed all three, and the feature had to be re-landed separately. Keep the flip in its own PR.

### Segment transport: SDK with bulk mode -> SDK with sync_mode -> hand-rolled HTTP batches
- **Files**: architecture.md, dependencies_and_packaging.md, testing.md
- **Evolution**: `StorageSegment` built on `segment-analytics-python`, with a `use_bulk` flag choosing between a 32KB regular limit and a 500MB bulk limit (#249) -> `use_bulk`/`BULK_MESSAGE_LIMIT` removed and chunk sizing fixed to measure the real wrapped-JSON size (#372) -> per-chunk `message_id = sha256(parent_id + chunk_index)` added via `segment_meta` for idempotent retries (#348) -> `analytics.sync_mode = True` to stop the SDK silently dropping >500KB background batches, with the note that "Segment silently rejects gzip-encoded bodies" (#383) -> the SDK dropped entirely for direct `requests.post` to `/v1/batch` with gzip on by default, self-built track envelopes, a 475,000-byte batch ceiling, pre-send oversized-event rejection, HTTPS-or-loopback host validation behind `allow_insecure_host`, and exceptions propagating to dispatcherd's retry machinery (#574). The `segment-analytics-python` dependency and its transitive packages left `pyproject.toml`/`uv.lock`; the Go mock server learned to decode gzip and record `Content-Encoding`.
- **PRs**: 5 (#249, #348, #372, #383, #574)
- **Lesson**: The gzip finding reversed -- what the SDK "silently rejected" the raw HTTP API accepts, so an SDK-observed limitation is a fact about the SDK, not the service. Each step here was an attempt to regain control the SDK was taking away (batch size, thread lifecycle, error visibility); once three workarounds have accumulated, replacing the SDK is cheaper than the fourth.

## AWX schema arcs (upstream dependency)

### main_host.last_job / last_job_host_summary FK columns: deprecate across releases, then drop
- **Files**: database_and_migrations.md
- **Evolution**: AWX bulk-updated `last_job_id` / `last_job_host_summary_id` on `main_host` at every job completion, which became a 75s query under scale-lab load (90:1 dead-to-live tuple ratio on `main_host`). #16332 (d1b3ae53ae) stopped all writes to the two FK columns and switched reads to the `with_latest_summary_id()` correlated subquery / `host.latest_summary` (derived from `main_jobhostsummary`), leaving the columns in place as dead weight -> #16530 (migration 0206) added a supporting composite index `main_jobhostsumm_host_id_desc` on `main_jobhostsummary(host_id, id DESC)` so the replacement latest-per-host query is an index-only top-1 scan, signaling AWX's commitment to the new pattern -> #16529 (migration 0210) **dropped** the columns: `RemoveField` for both FKs, `HostSerializer` now exposes them as `SerializerMethodField`s backed by `latest_summary`, the unused `JobHostSummary.latest_for_host`/`latest_job_for_host` classmethods were removed, and the `cleanup_jobs` `UPDATE main_host SET last_job_host_summary_id = NULL` step (which would have crashed on the dropped column) was deleted.
- **PRs**: 3 (#16332, #16530, #16529)
- **Impact on us**: `main_host` is a tracked collector dependency -- any collector still SELECTing `main_host.last_job_id` must derive the latest job from `main_jobhostsummary` (join `host_id`, take latest `id`, read `job_id`) before consuming AWX 2.7/devel schemas. Classic AWX deprecate-then-drop lifecycle: once a denormalized FK is deprecated, expect it to be dropped a release or two later.
- **Resolved in m-u**: fb75800 (#567) rewrote `_main_host_query()` to reach `main_unifiedjob` through a `LEFT JOIN LATERAL (SELECT job_id FROM main_jobhostsummary WHERE host_id = main_host.id ORDER BY id DESC LIMIT 1)`, and refreshed `tools/docker/latest.sql` to the post-0210 schema so the stale join can no longer pass tests. See metrics_collection.md.

### metrics-utility's pl/pgsql helpers: ours to install -> accidentally dumped -> adopted upstream by AWX
- **Files**: database_and_migrations.md, ci_cd.md, architecture.md
- **Evolution**: metrics-utility created `metrics_utility_parse_yaml_field` / `metrics_utility_is_valid_json` at query time via `ensure_functions(db)` (`CREATE OR REPLACE`), which needs write access to the controller database -- so the service (read-only) path had to avoid the collectors that call it, and GitHub CI (no write access) got a hand-maintained `tools/docker/functions.sql` loaded right after `latest.sql` -> e3df6b4 (#565) refreshed `latest.sql` to AWX devel and the diff was *nothing but* these two functions, read at the time as the dump having been taken from a database m-u had already written to -> b336400963 (awx#16458, migration `0211_create_metrics_utility_functions`) shows what really happened: AWX itself now creates both functions and `GRANT EXECUTE ... TO PUBLIC`, so any dump of a migrated AWX contains them, and the dumped bodies are AWX's hardened versions (escaped `field`, `WHEN invalid_text_representation`) rather than our older ones. Open m-u PR #577 closes the CI half by adding `ON_ERROR_STOP` and deleting the duplicate `functions.sql`.
- **PRs**: 3 (m-u #565, awx#16458, m-u #577 open)
- **Impact on us**: No table, column or index changes -- but the read-only-connection blocker is gone on AWX >= 0211, and `functions.sql` is now dead weight. The unresolved half is `ensure_functions()`: its `CREATE OR REPLACE` will silently downgrade AWX's escaped/narrow-catch definitions to our old unescaped/`WHEN others` ones on any database where m-u has write access. It needs to become version-aware or go away.
- **Lesson**: Vendoring a helper upstream eventually adopts is a win only if you then stop installing your own copy. Three copies of the same function (migration, CI fixture, runtime installer) drifting in two directions is the failure mode, and `CREATE OR REPLACE` guarantees the *last* writer wins rather than the newest definition.
