...
- **Insight**: This endpoint gives the UI a single call to check whether dashboard collection is active and what state it's in, avoiding multiple API calls. Notably, both commits were pushed directly without PR numbers, suggesting rapid iteration on a feature branch or direct commits to devel. **The permission model here was later replaced** by 09dfeaf (#410): the viewset dropped `BaseAdminViewSet` for a plain `ViewSet` + `IsAuthenticated` with per-action admin checks, gained `show_gamification`/`show_dashboard` fields and a POST action -- see "collection_status opened to all authenticated users, with per-action admin checks and a POST flag toggle" below.
...
### Automation leaderboards API: one read-only aggregate endpoint, identities reduced to initials
- **Repo**: ansible/metrics-service
- **Commits**: 7734e5c (#422), ac7f79e (#435)
- **What happened**: A new `leaderboard` route on the dashboard_reports router (`DashboardLeaderboardsViewSet`) returns a single aggregate object for the trailing 30 UTC days: `job_runs`, `active_organizations`, `featured_template`, `enterprise_streak`, `org_streak`, `organization_leaderboard`, three `activity_levels` (volume / breadth / consistency, each with a top 10, the caller's `current_user_rank` and `total_users` so the UI can say "rank N of X"), `user_achievements` and `org_achievements`. It is a `ReadOnlyModelViewSet` in name only: `list()` is fully overridden and ignores query params, `retrieve()` is `@extend_schema(exclude=True)` and returns 405, `queryset = JobData.objects.none()` exists purely to satisfy schema tooling, `pagination_class = None`, and `filter_backends = []` so DAB's default filter/ordering/search backends do not advertise `JobData` query parameters the endpoint never reads. Because the response is an object rather than a list, the schema is declared with drf-spectacular's `forced_singular_serializer(DashboardLeaderboardsSerializer)`. Permission is plain `IsAuthenticated` -- privacy comes from the payload instead: ranking and identity are by `launched_by_id`, the caller sees their own username and an `is_current_user: true` marker, and every other user is shown as the first two letters of their username upper-cased. A ~600-line unit suite (`TestAuth`, `TestCounts`, `TestEnterpriseStreak`, `TestOrganizationLeaderboard`, `TestActivityLevels`, `TestUserAchievements`, `TestOrgAchievements`, `TestEdgeCases`, `TestResponseShape`) drives it against a frozen `datetime` subclass.
- **Insight**: A "leaderboard" endpoint is an aggregate resource, not a collection -- overriding `list()` on a `ReadOnlyModelViewSet` needs the router-facing knobs explicitly neutralised (`queryset.none()`, `pagination_class = None`, `filter_backends = []`, `forced_singular_serializer`) or the generated OpenAPI advertises pagination, filters and a list shape the endpoint does not honour. Showing other users as two-letter initials keeps the endpoint open to every authenticated user without an extra permission tier.

### collection_status opened to all authenticated users, with per-action admin checks and a POST flag toggle
- **Repo**: ansible/metrics-service
- **Commits**: 09dfeaf (#410)
- **What happened**: `DashboardCollectionStatusViewSet` moved off `BaseAdminViewSet` (class-wide `IsSystemAdminOrAuditor`) to a plain DRF `ViewSet` with `permission_classes = [IsAuthenticated]`, evaluating `IsSystemAdminOrAuditor().has_permission(request, self)` inline in each action instead. `list()` gained two fields: `show_gamification` (the raw flag, visible to everyone) and `show_dashboard`, which is `get_feature_enabled_from_db("SHOW_DASHBOARD", default=True) and is_system_admin_or_auditor` -- i.e. flag AND role, computed per request. A new `create()` (POST) lets a system admin or auditor flip `SHOW_GAMIFICATION` at runtime: it raises `PermissionDenied` for anyone else, `ValidationError` when the body value is not a real bool (`isinstance(value, bool)`, so `"true"` is rejected), and otherwise `Setting.objects.update_or_create(setting_key="SHOW_GAMIFICATION", defaults={"current_value": json.dumps(value), "last_modified_by": request.user})`. Adding the second action forced the inline schema serializer to be renamed `DashboardCollectionResponse` -> `DashboardCollectionListResponse`, with `DashboardCollectionPostRequest`/`...PostResponse` alongside.
- **Insight**: When one endpoint must serve both regular users and admins, dropping the class-level permission to `IsAuthenticated` and re-checking the admin predicate inside each action lets a single response carry public fields plus role-derived ones -- at the cost that every future action has to remember the check, since the class no longer enforces it. Note also that `inline_serializer` names are global to the schema, so a second action on the same viewset usually means renaming the first one's serializer.
...
### DAB DefaultPaginator ignores REST_FRAMEWORK PAGE_SIZE; custom subclass needed
- **Repo**: ansible/metrics-service
- **Commits**: 69a6327 (#337)
- **What happened**: DAB's `DefaultPaginator.__init__` reads its own `DEFAULT_PAGE_SIZE` / `MAX_PAGE_SIZE` Django settings (falling back to ansible_base's hardcoded 50/200) instead of the project's `REST_FRAMEWORK['PAGE_SIZE']`. Dashboard report endpoints were paginating at the wrong default size. A `DashboardReportPagination(DefaultPaginator)` subclass was created that pins `self.page_size = api_settings.PAGE_SIZE` in `__init__`. Additionally, an `AliasedOrderingFilter(OrderingFilter)` was introduced to accept both DRF's standard `ordering` query param and a UI-friendly `order_by` alias, with field name remapping via a view's `ordering_field_aliases` dict (e.g., mapping a user-facing name to the actual queryset column). The filter reimplements `get_ordering()` rather than patching `request.query_params` (which is a read-only `QueryDict`).
- **Insight**: DAB's paginator and DRF's `api_settings.PAGE_SIZE` are independent configuration paths -- when using DAB's paginator, always verify the effective page size matches the project's REST_FRAMEWORK config, and subclass if needed. For ordering aliases, reimplementing the method is safer than patching the read-only request object.

### FilterSet duplicate name validation at serializer level
- **Repo**: ansible/metrics-service
- **Commits**: 65a8cfe (#338)
...
- **Repo**: ansible/metrics-service
- **Commits**: bb9b374 (#357)
- **What happened**: DAB's `EntryReadOnlyViewSet` (activitystream) and `OldFeatureFlagsStateListView` (feature_flags) were accessible to any authenticated user, regardless of RBAC role. These views are registered under DAB's URL patterns before the project's own `apps/urls.py`, so URL-level shadowing cannot take precedence. Fix: `CoreConfig.ready()` patches `permission_classes = [IsSystemAdminOrAuditor]` on both view classes at app startup. For `UserViewSet`, a `get_permissions()` override applies per-action permissions: `IsAuthenticated` for `/me` (self-profile), `IsSystemAdminOrAuditor` for list/retrieve, and both `IsSystemAdminOrAuditor` + `AnsibleBaseUserPermissions` for write operations. Thirteen new RBAC tests cover basic-user denial, platform-auditor access, superuser access, and unauthenticated denial across all three endpoints.
- **Insight**: When a framework (DAB) registers URL patterns before the project's own URL configuration, you cannot override its views by URL shadowing. Class-attribute patching in `AppConfig.ready()` is the clean alternative -- it modifies the actual view class that handles the request. This is preferable to middleware-level checks because it integrates with DRF's permission system and appears in OpenAPI documentation.

### Organization POST blocked to prevent DAB post_save signal crash
- **Repo**: ansible/metrics-service
- **Commits**: 412bfe6 (#361)
- **What happened**: Organizations are managed by the AAP resource server and synced into metrics-service (`is_provider=False` in `resource_api.py`). POSTing to `/api/v1/organizations/` triggered DAB's `sync_to_resource_server_post_save` signal, which crashed because the local service is not the provider and the signal is not designed for consumer-side creation. Fix: `OrganizationViewSet.http_method_names` excludes `"post"`, returning 405 Method Not Allowed before the signal path is reached. Tests verify: superusers get 405 on POST (method guard fires after auth), non-superusers get 403 (RBAC fires before the method guard).
- **Insight**: When a model is synced from an upstream resource server (`is_provider=False`), block creation at the HTTP method level (`http_method_names`) rather than in serializer validation or a custom permission class. The method guard fires early in the DRF dispatch chain and returns a clear 405, preventing the request from reaching model.save() and its problematic signals. Document the constraint in the viewset docstring with a reference to the resource_api.py provider flag.
