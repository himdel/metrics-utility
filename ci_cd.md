...
- **NOTE**: The original `sync-openapi-specs.yml` in this entry never worked as written; it was reworked three times (#394/#396/#397) — see the next entry. Details here (`AAP_OPENAPI_SPECS_PAT` secret, `metrics-service/openapi-schema.json` output, hardcoded `main` target, single job) are superseded.

### sync-openapi-specs workflow reworked to match the handbook drift-detection standard (three iterations)
- **Repo**: ansible/metrics-service
- **Commits**: 7a99030e (#394), dfc0502f (#396), 502534c3 (#397)
- **What happened**: The `sync-openapi-specs.yml` workflow from #196 was broken and iterated three times. (1) **#394 (make it run)**: fixed `DJANGO_SETTINGS_MODULE` from the nonexistent `apps.core.settings` to `metrics_service.settings` (the `manage.py` entry point); switched env vars to the `METRICS_SERVICE_` Dynaconf prefix; replaced `DATABASE_URL` with per-database SQLite overrides so `manage.py spectacular` runs without a real Postgres; output the spec as a flat `metrics.json` at repo root (matching the aap-openapi-specs flat-file convention) instead of `metrics-service/openapi-schema.json`; targeted the matching versioned branch in aap-openapi-specs instead of hardcoded `main`; added `stable-2.6`/`stable-2.7` triggers; and split into a **generate** job (runs upstream + downstream, validating the schema on every push) and a **sync** job (runs only downstream, guarded by `github.repository`, consuming the generated artifact). Also added workflow `permissions` per a CodeQL finding. (2) **#396 (align to handbook)**: complied with the [OpenAPI drift-detection handbook standard](https://handbook.eng.ansible.com/Quality/openapi-drift-detection) — `LC_ALL=C.UTF-8` at workflow level; `stable-*` glob trigger; artifact renamed to `openapi-schema` downloaded to `./component-spec/`; added verify/branch-exists/fail-if-missing steps; replaced `git diff` (wrong — file not in index until after copy) with `diff -q` comparing the component spec directly against the spec-repo file, handling new-file vs update via an `is_new_file` output; secret renamed `AAP_OPENAPI_SPECS_PAT` -> `OPENAPI_SPEC_SYNC_TOKEN`; branch naming switched from timestamp to 7-char short SHA; PR title/body aligned to the handbook template. Also fixed a **critical shell-injection**: `github.ref_name/sha/repository` and `head_commit.message` were interpolated directly into shell source, now passed exclusively through step `env` vars (`SOURCE_BRANCH`, `SOURCE_SHA`, `SOURCE_REPO`, `HEAD_COMMIT_MSG`, `IS_NEW_FILE`). (3) **#397 (verified commits)**: imported `OPENAPI_SPEC_SYNC_GPG_PRIVATE_KEY` and configured git to GPG-sign commits as `aap-api-bot`, matching the ansible/tower (Controller) spec-sync pattern.
- **Insight**: A cross-repo spec-sync workflow has three separable concerns that were each gotten wrong once — (a) making the generator actually run headless (correct settings module, Dynaconf-prefixed env vars, SQLite so no Postgres is needed), (b) conforming to the shared aap-openapi-specs conventions (flat-file naming, versioned-branch targeting, handbook artifact/PR/secret names, `diff -q` not `git diff` for a not-yet-indexed file), and (c) trust/security (never interpolate `github.*` context into shell — route through `env` — and GPG-sign with the shared bot key for verified downstream commits). Split generate/sync jobs so schema validation runs on every push in both repos while the actual downstream PR only fires from the canonical repo.
...
### Push-triggered downstream repo sync workflow to ansible-automation-platform
- **Repo**: ansible/metrics-utility
- **Commits**: 82d25fe (#563)
- **Also**: ansible/metrics-service — cb101c6 (#416), an essentially identical `.github/workflows/repo-sync.yaml` mirroring `devel` to `ansible-automation-platform/metrics-service`. Same org-standard template, same org secret/variable, same generic `TARGET_REPO: ${{ github.event.repository.name }}` parameterization (so the file is repo-agnostic), same hardening.
- **What happened**: A new `.github/workflows/repo-sync.yaml` mirrors `devel` from `ansible/metrics-utility` to `ansible-automation-platform/metrics-utility` on every push to `devel`, supplementing (not replacing) the existing hourly scheduled sync in `ansible-automation-platform/cicd-ops` so merges mirror downstream immediately. It follows the org-standard caller template from `ansible/ansible-cicd`'s `i-want-it-now.yaml`: it mints a scoped GitHub App token via `actions/create-github-app-token@v2` (using org secret `ANSIBLE_CICD_BOT_ORG_RW_TOKEN_PRIVATE_KEY` and org var `ANSIBLE_CICD_BOT_ORG_RW_TOKEN_APP_ID`, restricted to the target org/repo), derives the bot's git identity from the app slug + user id, then clones the source single-branch with the run's `github.token` and pushes `HEAD:devel` downstream with the app token. Hardened with `permissions: contents: read`, a `concurrency` group per ref with `cancel-in-progress`, a 10-minute timeout, and an `always()` cleanup step.
- **Insight**: A push-triggered mirror gives immediate downstream propagation on top of a slower scheduled fallback; using a scoped GitHub App token (minted per-run, restricted to the single target repo) rather than a broad PAT is the org-standard way to grant cross-org write, and separating the read (source clone with `github.token`) from the write (downstream push with the app token) keeps each credential minimally scoped. Because the workflow parameterizes the target off `github.event.repository.name`, the same file drops into both metrics-utility and metrics-service unchanged.
