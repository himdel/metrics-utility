...
### Indirect node collector default-enabled in `METRICS_UTILITY_OPTIONAL_COLLECTORS`
- **Repo**: ansible/metrics-utility
- **Commits**: 76aadfd (#560)
...
### Report type and CCSP sheets default-enabled; `METRICS_UTILITY_REPORT_TYPE` no longer required
- **Repo**: ansible/metrics-utility
- **Commits**: 902ce04 (#559)
- **What happened**: For indirect node GA (ANSTRAT-1628 / AAP-87148), two new helpers were added to `base/utils.py`: `get_report_type()` defaults `METRICS_UTILITY_REPORT_TYPE` to `'CCSPv2'` instead of raising when unset (build_report stopped raising `MissingRequiredEnvVar`; validation stopped erroring on empty), and `get_optional_ccsp_report_sheets(report_type)` centralizes the default sheet list that was previously duplicated (hardcoded) in both `build_report.py` and `validation.py`. The default sheets now include `indirectly_managed_nodes` for both CCSP and CCSPv2, plus `infrastructure_summary` inserted into the default *only* for CCSPv2 (CCSPv1 can't render it). `infrastructure_summary` was added to `VALID_SHEETS['CCSP']` so that if a customer explicitly requests it under CCSPv1, `report_ccsp.py` logs a visible warning and skips just that sheet rather than hard-failing the whole report build. Sheets render gracefully with empty indirect-node data, so this PR is safe even before the collector default (#560) lands.
- **Insight**: Making an env var optional-with-default plus centralizing a previously-duplicated default list into one helper keeps build and validation in sync; allowing a not-really-valid sheet through validation specifically so the report can degrade with a logged warning (instead of a hard failure) is a deliberate graceful-degradation choice for a config a customer might override anyway.
