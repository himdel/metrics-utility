...
### S3 access/secret key env vars made optional to support implicit credentials (IRSA, instance profiles)
- **Repo**: ansible/metrics-utility
- **Commits**: a80e9d4 (#547)
- **What happened**: `METRICS_UTILITY_BUCKET_ACCESS_KEY` and `METRICS_UTILITY_BUCKET_SECRET_KEY` became optional so that boto3's default credential chain (IRSA, EC2 instance profiles, `~/.aws/credentials`, etc.) can resolve credentials automatically when they are omitted. Three layers changed: (1) In `management/validation.py`, `handle_s3_ship_target()` no longer requires each key individually -- instead it only errors if exactly one of the pair is set (`bool(access) != bool(secret)`), and the comment marks both keys plus region as optional. (2) `S3Handler` (billing) and `StorageS3` (library) both raise `ValueError` in `__init__` if only one of the pair is provided, and their lazy session builders only pass `aws_access_key_id`/`aws_secret_access_key` to `boto3.Session` when both are present. (3) When keys are omitted, they eagerly call `session.get_credentials()` and raise a clear `ValueError` (naming both env vars and the implicit options) if boto3 cannot locate any credentials, rather than failing later with an opaque error at request time.
- **Insight**: To support implicit cloud credentials, omit the access/secret kwargs entirely (don't pass empty strings) so boto3's default chain kicks in -- but reject mismatched half-configured pairs and eagerly verify credentials resolve, turning a late opaque failure into an actionable early error.
...
### `INDIRECT_NODE_COLLECTION` feature flag default-enabled (metrics-service side)
- **Repo**: ansible/metrics-service
- **Commits**: 53b0835 (#413)
- **What happened**: For the GA of indirect node collection in AAP 2.7 (AAP-87154, part of ANSTRAT-1628), the `INDIRECT_NODE_COLLECTION` feature flag was flipped from the previous `False`/customer-opt-in default to `True`. The flag was added explicitly to the `FEATURE` dict in `apps/settings/defaults.py` (previously it was absent, defaulting off), the `INDIRECT_NODE_COLLECTION_GROUP` header comment in `apps/tasks/task_groups.py` was updated to say "default: True; enabled by default -- disable with `METRICS_SERVICE_FEATURE__INDIRECT_NODE_COLLECTION=false` or dynamic_settings.Setting", and the four docs tables (`README.md`, `docs/README.md`, `docs/dynamic-settings.md`, `docs/task-system.md`) were flipped from `false (opt-in)` to `true`. The override paths (env var, installer flag, dynamic DB setting) are unchanged, so operators can still disable it. This is the metrics-service (collection-scheduling) half of the same GA that default-enabled the `main_indirectmanagednodeaudit` collector on the metrics-utility side (76aadfd #560 / AAP-87149) -- **note that the metrics-utility half was later reverted (#583) while this flag stayed `True`**. The two are separate code paths (this flag gates metrics-service's `INDIRECT_NODE_COLLECTION_GROUP` tasks; `METRICS_UTILITY_OPTIONAL_COLLECTORS` gates the metrics-utility CLI's `gather`, and metrics-service never reads it), so neither is broken by the other -- but the "GA on by default" story now only holds on the metrics-service side. See "Indirect node GA defaults reverted" above and the Superseded entry for #560.
- **Insight**: Default-enabling a cross-repo feature at GA takes coordinated flips on both sides -- metrics-service turns on the `INDIRECT_NODE_COLLECTION` scheduling flag while metrics-utility default-includes the underlying collector -- and the flag should be spelled out explicitly in the `FEATURE` defaults dict (not left to implicit-absent-is-false) so the enabled default and its disable path are self-documenting.

### Indirect node GA defaults reverted -- everything stays opt-in
- **Repo**: ansible/metrics-utility
- **Commits**: 67220f6 (#579), c923cbd (#581), 3366d2e (#582), a6ba967 (#583)
- **What happened**: The AAP-87148/87149/87150 "default-enable indirect nodes for GA" work (#559 902ce04, #560 76aadfd) was rolled back in a four-commit revert-of-a-revert chain over four days. #579 reverted the #559/#560 default flips; #581 reverted #579 (putting the defaults back); then #582 reverted #559 outright and #583 reverted #560 outright. The net state at HEAD is a full return to the pre-GA behaviour: `get_optional_collectors()` defaults to `'main_jobevent'` only (no `main_indirectmanagednodeaudit`); `get_report_type()` and `get_optional_ccsp_report_sheets()` were deleted from `base/utils.py` entirely, so `build_report.py` and `validation.py` are back to reading `os.getenv('METRICS_UTILITY_REPORT_TYPE')` directly with `MissingRequiredEnvVar` raised when unset (the env var is **required** again, no CCSPv2 default); the default sheet list is back to the hardcoded-in-two-places `'ccsp_summary,managed_nodes,usage_by_organizations,usage_by_collections,usage_by_roles,usage_by_modules'` with neither `indirectly_managed_nodes` nor `infrastructure_summary` in it; and `validate_collectors()` is back to its own raw `os.getenv(...)` copy of the default. All the CCSP/CCSPv2 snapshot `report.xlsx` fixtures shrank back to the no-indirect-sheet baseline. Deliberately **kept** through the whole chain: the AAP-89203 indirect-nodes-in-`Usage by Collections` implementation (#558 c00f979) -- #579's PR body explicitly listed "preserves the AAP-89203 Usage by Collections implementation" as a requirement and included a `git diff --exit-code devel...HEAD -- report/base.py report_ccsp.py report_ccsp_v2.py` check to prove it. `infrastructure_summary` was separately re-added to `VALID_SHEETS['CCSP']` right afterwards by #584.
- **Insight**: Reverting a "flip the defaults at GA" change is not a single `git revert` -- the feature code (indirect nodes in Usage by Collections) had to stay while only the default-enablement was undone, and the revert also un-did the incidental refactor that de-duplicated the sheet-list default, so `build_report.py` and `validation.py` once again each hold their own copy of the same default string. Bundling a cleanup refactor into a policy-flip PR means the cleanup dies with the flip.

### `infrastructure_summary` allowed through CCSP validation so the report can warn and skip
- **Repo**: ansible/metrics-utility
- **Commits**: 574840b (#584)
- **What happened**: Re-landed (standalone, after the #582 revert removed it) the graceful-degradation path for `infrastructure_summary` under CCSP v1: `'infrastructure_summary'` was added back to `VALID_SHEETS['CCSP']` in `validation.py`, and `ReportCCSP._build_spreadsheet()` logs a `logger.warning` naming the sheet, saying it is CCSPv2-only, and pointing at both remedies (`METRICS_UTILITY_REPORT_TYPE=CCSPv2`, or drop it from `METRICS_UTILITY_OPTIONAL_CCSP_REPORT_SHEETS`) before skipping it. A focused test asserts both halves: the warning appears in `caplog.messages` **and** `'Infrastructure Summary' not in wb.sheetnames`.
- **Insight**: When a config value is valid for one variant of an output and meaningless for another, the friendliest contract is "validation accepts it, the renderer warns and skips it" -- but the warning has to name the env var that would fix it, and the test has to assert the skip as well as the warning, otherwise a future refactor can keep the log line while quietly rendering the sheet.

### SHOW_GAMIFICATION and SHOW_DASHBOARD: one admin-writable runtime flag, one flag ANDed with the caller's role
- **Repo**: ansible/metrics-service
- **Commits**: 09dfeaf (#410)
- **What happened**: Two gamification-related flags were added, both read through the existing five-tier `get_feature_enabled_from_db()` lookup but with different semantics. `SHOW_GAMIFICATION` (default `False`) controls whether the UI shows the gamification tabs and is the first flag the service lets you flip through its own API: `POST /collection_status/` writes a `Setting` row (`current_value=json.dumps(bool)`, `last_modified_by=request.user`) via `update_or_create`, taking effect immediately with no restart, and is restricted to system admins/auditors. `SHOW_DASHBOARD` (default `True`) is never written by the API -- `list()` returns `get_feature_enabled_from_db("SHOW_DASHBOARD", default=True) and is_system_admin_or_auditor`, so a regular AAP user sees `show_gamification: true, show_dashboard: false` while an admin sees both. The product rationale in the PR: regular users should get the gamification tabs but not the dashboard tabs.
- **Insight**: Two flags that look alike in `defaults.py` can mean quite different things -- one is persisted operator state (Setting row, admin-writable, restart-free), the other is a per-request capability hint where only the flag half is stored and the role half is evaluated per caller. Worth naming the difference, because only the first kind is meaningfully "toggleable".

## Superseded / Semi-Obsolete

### Indirect node collector default-enabled in `METRICS_UTILITY_OPTIONAL_COLLECTORS`  [REVERTED by #583]
- **Repo**: ansible/metrics-utility
- **Commits**: 76aadfd (#560); reverted by a6ba967 (#583)
- **What happened**: For the GA of indirect nodes in AAP 2.7 (ANSTRAT-1628 / AAP-87149), `main_indirectmanagednodeaudit` was added to the default value of `METRICS_UTILITY_OPTIONAL_COLLECTORS` in `get_optional_collectors()` -- the default went from `'main_jobevent'` to `'main_jobevent,main_indirectmanagednodeaudit'` -- so indirect audit data is collected without any env var configuration. This is backward compatible because the collector already try/excepts a missing `main_indirectmanagednodeaudit` table (older controllers), and the env var override still works. The change also fixed drift: `validate_collectors()` in `validation.py` had independently hardcoded the old `'main_jobevent'` default via a raw `os.getenv(...)`; it now calls `get_optional_collectors()` directly so the default lives in exactly one place. **All of this was reverted**: the default is `'main_jobevent'` again and `validate_collectors()` is back to its own hardcoded `os.getenv(...)` copy, so the getter/validator drift is back too.
- **Insight**: Still true that duplicated defaults drift -- but the de-duplication only survived while the default flip it rode in on survived. Collecting `main_indirectmanagednodeaudit` requires an explicit `METRICS_UTILITY_OPTIONAL_COLLECTORS` setting again.

### Report type and CCSP sheets default-enabled; `METRICS_UTILITY_REPORT_TYPE` no longer required  [REVERTED by #582]
- **Repo**: ansible/metrics-utility
- **Commits**: 902ce04 (#559); reverted by 3366d2e (#582)
- **What happened**: For indirect node GA (ANSTRAT-1628 / AAP-87148), two new helpers were added to `base/utils.py`: `get_report_type()` defaults `METRICS_UTILITY_REPORT_TYPE` to `'CCSPv2'` instead of raising when unset (build_report stopped raising `MissingRequiredEnvVar`; validation stopped erroring on empty), and `get_optional_ccsp_report_sheets(report_type)` centralizes the default sheet list that was previously duplicated (hardcoded) in both `build_report.py` and `validation.py`. The default sheets now include `indirectly_managed_nodes` for both CCSP and CCSPv2, plus `infrastructure_summary` inserted into the default *only* for CCSPv2 (CCSPv1 can't render it). `infrastructure_summary` was added to `VALID_SHEETS['CCSP']` so that if a customer explicitly requests it under CCSPv1, `report_ccsp.py` logs a visible warning and skips just that sheet rather than hard-failing the whole report build. **Both helpers were deleted by the revert**: `METRICS_UTILITY_REPORT_TYPE` is a required env var again, the sheet-list default is hardcoded in two places again, and neither `indirectly_managed_nodes` nor `infrastructure_summary` is on by default. Only the warn-and-skip path survived, re-landed standalone in 574840b (#584).
- **Insight**: Superseded by the revert chain (#579/#581/#582/#583) -- see "Indirect node GA defaults reverted" in the active section. The graceful-degradation idea outlived the default flip because it was separable; the refactor that centralized the default list did not, because it was entangled with it.
...
### Environment docs caught up with Candlepin, standalone DB and implicit S3 credentials
- **Repo**: ansible/metrics-utility
- **Commits**: ac2d48b (#562)
- **What happened**: `docs/environment.md` was missing a whole cluster of env vars that had accumulated in code. Added: the nine Candlepin ones (`METRICS_UTILITY_CANDLEPIN_CA`, `_CERT_DIR`, `_ENABLED`, `_LIFECYCLE_ENABLED`, `_ORG`, `_REGISTRATION_ENABLED`, `_RENEWAL_DAYS`, `_STORAGE`, `_URL`), the standalone/mock-AWX Controller DB settings (`METRICS_UTILITY_DB_NAME`/`_PASSWORD`/`_PORT`/`_USER`, previously only `_DB_HOST` was documented), and `METRICS_UTILITY_RH_USERNAME`/`_RH_PASSWORD`. Two descriptions were corrected rather than added: `METRICS_UTILITY_PROXY_URL` is an HTTP/HTTPS proxy for both CRC uploads *and* Candlepin calls (it was documented as "upload proxy"), and the S3 section now states that `METRICS_UTILITY_BUCKET_ACCESS_KEY`/`_SECRET_KEY` are optional when the runtime resolves credentials through the default boto3 chain (IRSA, EC2 instance profile) -- but if one is set, both must be.
- **Insight**: An env-var list drifts silently because nothing fails when a variable is undocumented; the entries worth the most are the ones that correct a wrong description (a proxy shared with a second subsystem) or record an implicit behaviour (credentials that may legitimately be absent because boto3 resolves them elsewhere).
