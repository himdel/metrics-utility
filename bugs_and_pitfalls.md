...
### Service metadata probe must authenticate without the RBAC claims fetch (circular service_id NULL)
- **Repo**: ansible/metrics-service
- **Commits**: e912984 (#405)
- **What happened**: On operator deployments upgraded 2.6->2.7, the gateway's `ServiceCluster.service_id` for metrics stays NULL (metrics registers after the gateway's `migrate_service_data` runs). The gateway's `populate_service_id` self-heal probes `GET /api/v1/service-index/metadata/` to read the service_id back, but metrics-service's default RBAC auth class (`use_rbac_permissions=True`) triggers a synchronous `jwt_claims` call back to the gateway, signed with the still-unresolvable service token, which 503s -> the probe 403s -> service_id stays NULL forever. Circular. Fix: a new `ServiceJWTAuthenticationNoRBAC` (validates the DAB JWT but sets `use_rbac_permissions=False`, skipping `process_rbac_permissions` and thus the recursive `jwt_claims` fetch), applied to `ServiceMetadataView` only via the existing `CoreConfig.ready()` class-attribute patching pattern. Every other endpoint keeps `use_rbac_permissions=True`. Verified against gateway source that `populate_service_id` only calls `get_service_metadata()` (metadata/), so `ServiceIndexRootView` needed no change. The OpenAPI schema was regenerated: the new authenticator has no `OpenApiAuthenticationExtension`, so the cookieAuth/basicAuth security block was dropped from the metadata path.
- **Insight**: An endpoint used by the gateway to bootstrap/self-heal a service's own `service_id` cannot use auth that re-calls the gateway (RBAC claims sync), or you get a deadlock where the probe can never succeed while service_id is NULL — carve out a no-RBAC JWT authenticator for that single bootstrap endpoint. This is the same class-attribute patching pattern (bb9b374 #357) used to harden DAB views, and closely related to the `service_type`/`service_id` registration pitfall (110d01e #313).

### FQCN role regex: nested quantifier captured the wrong segment, and the fix only reached the dead copy
- **Repo**: ansible/metrics-utility
- **Commits**: 61fd906 (#556)
- **What happened**: `DataframeContentUsage.collection_regexp()` used `r'^(\w+)\.(\w+)\.((\w+)(\.|$))+'`. For a 4+ segment FQCN the repeated group leaves `groups()[2]` holding the *last* repeated segment, not the third -- so `ansible.builtin.copy.something` produced role `ansible.builtin.something` instead of `ansible.builtin.copy`. The nested quantifier `((\w+)(\.|$))+` is also the catastrophic-backtracking (reDOS) shape Snyk flagged back in #342. #342 rewrote exactly this regex to the linear `r'^(\w+)\.(\w+)\.(\w+)(?:\.\w+)*$'`, but only in the `library/` copy of the dataframe engine -- the duplicated CLI copy in `automation_controller_billing/dataframe_engine/dataframe_content_usage.py` was never patched, and #540 then deleted the library copy, so the fixed version disappeared while the buggy one stayed live. #556 backported the linear pattern to the CLI copy. Collection extraction (first two segments) is unchanged; the anchored `$` means malformed role strings with trailing non-word junk (`ansible.builtin.copy.more extra`) now fail to match and fall back to "No role used" instead of yielding a bogus partial. Scope: the `main_jobevent` content-usage rollup's `role_name` column in CCSP / CCSPv2 reports only; RENEWAL_GUIDANCE is unaffected.
- **Insight**: Two lessons in one bug -- a repeated capture group `((\w+)(\.|$))+` returns only the last iteration, so it is never the right way to grab the Nth segment; and when a security or correctness fix lands on one of two duplicated copies, verify which copy is actually reachable, because deleting the "unused" copy later silently reverts the fix.

### Event data `replace(event_data, '\u', '\u')` is load-bearing, and the host collectors have the same json-vs-jsonb gap
- **Repo**: ansible/metrics-utility
- **Commits**: 049313e (#566)
- **What happened**: `docs/event-data-unicode-escaping.md` was written to settle whether the odd `replace(event_data, '\u', '\u005cu')::jsonb` in the `main_jobevent` collectors is an obsolete copy-paste. It is not. AWX stores `main_jobevent.event_data` as `text` (a deliberate legacy choice to avoid rewriting millions of rows at upgrade, see `JSONBlob` in `awx/main/fields.py`). PostgreSQL `json` input validation only checks that `\u` is followed by four hex digits, while `jsonb` additionally rejects `\u0000` (NUL cannot exist in PostgreSQL `text`), lone/mispaired UTF-16 surrogates, and escapes unrepresentable in the database encoding. The history: AWX #9279 (2021) hit NUL in job output; #9821 retried collection after stripping `\u0000` on `UntranslatableCharacter`; `e3893b18` moved analytics from `json` to `jsonb` for the `- 'artifact_data'` operator; #12252 (2022) replaced the retry with unconditional neutralization of every `\u` escape. metrics-utility copied the expression in 9272f1d (#17) without discussion. The doc also records the deliberate side effects -- the replacement is not JSON normalization, it mangles valid escapes too (`"\u00e9"` becomes the literal text `\u00e9`, not `é`) -- which is why it must stay event-scoped and must not be folded into `metrics_utility_is_valid_json()`. And it names a live latent bug: the host collectors (`job_host_summary`, `main_host`, `main_host_daily`, `main_hostmetric`) guard with `metrics_utility_is_valid_json(main_host.variables)` (a `json` cast) but then extract with `main_host.variables::jsonb->>'ansible_host'`, so a value containing `\u0000` or a lone surrogate passes the guard and aborts the whole collector query. Switching extraction to `::json` is not a fix -- the extraction operators materialize the escapes and raise the same errors.
- **Insight**: PostgreSQL `json` and `jsonb` do not accept the same inputs, so a validity guard written against `json` cannot protect a cast or extraction that uses `jsonb`; when you inherit a defensive workaround from upstream, write down what it protects against and what it breaks before deciding it is dead code.

### Empty-string cron expressions counted as recurring, so one-shot tasks were never cleaned up, run or retried
- **Repo**: ansible/metrics-service
- **Commits**: ac7f79e (#435)
- **What happened**: `Task.cron_expression` has two ways to say "not recurring" -- `NULL` and `""` -- and every query used only `cron_expression__isnull`. Consequences: `cleanup_old_tasks(preserve_recurring=True)` treated empty-cron completed/failed tasks as recurring and kept them forever (on both the `completed_at` path and the `modified` fallback); `recurring_tasks()` handed them to the scheduler as if they had a schedule; `ready_to_run()` and `immediate_tasks()` skipped them, so they never ran and failed ones were never retried. The cleanup test had been asserting the buggy behaviour on purpose, with a `FIXME: This test will fail until we also update the cleanup filter` and `assert result["tasks_deleted"] == 1`; the fix flips it to `== 3` and adds a task exercising the `modified` fallback. Root fix is the shared `Task.non_recurring_filter()` predicate (see task_system.md). An earlier partial fix had only stopped the *serializer* from writing empty strings for new tasks, leaving existing rows stuck.
- **Insight**: Fixing the writer ("stop creating empty strings") does not fix the readers -- rows already in that state stay invisible to every `isnull` filter. When a nullable text column acquires a second spelling of "absent", either normalise it at the database level or make every predicate accept both, and treat a test that documents the bug with a FIXME as an open ticket, not as coverage.

### Leaderboard org tie-break sorted NULL-named orgs first, contradicting the documented order
- **Repo**: ansible/metrics-service
- **Commits**: ac7f79e (#435)
