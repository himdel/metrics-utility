...
### Mock Prometheus server for vCPU collector integration tests
- **Repo**: ansible/metrics-utility
- **Commits**: 29e4073 (#432)
- **What happened**: A mock Prometheus HTTP server was added in Go (`tools/mock-prometheus-server/main.go`, ~326 lines) following the mock-segment-server pattern. It implements `/api/v1/query` (instant) and `/api/v1/query_range` (range queries) with configurable CPU values via `POST /config` (supports both single string and array of strings -- range queries cycle through the list per data point). Utility endpoints: `POST /reset` (restore defaults), `GET /requests` (return all captured requests). Both mock servers (Segment and Prometheus) were refactored together: switched from manual router switch to `http.NewServeMux` (Go 1.22+), `--port` flags replaced with env vars only, mock-segment had file output removed (memory-only via `/requests`). Integration tests in `test_collectors_prometheus_client_integration.py` exercise `PrometheusClient`, `get_total_workers_cpu`, and `get_cpu_timeline` against the real HTTP server. A separate `test_vcpu_gather_integration.py` tests the full vCPU gather flow. CI builds and starts both mock servers in a single step. Docker Compose added services for both mocks with `MOCK_SEGMENT_URL`/`MOCK_PROMETHEUS_URL` env vars for container networking.
- **Insight**: The mock server pattern (Go binary with configurable responses, request capture, and reset) is now the standard approach for testing external service integrations in this project -- it's been applied to both Segment and Prometheus, and the shared patterns (ServeMux routing, POST /reset, /requests endpoint, env-var-based URLs) form a reusable template.

### OperationalError vs ProgrammingError: different error types require different handling strategies
- **Repo**: ansible/metrics-utility
- **Commits**: 992db11 (#477)
- **What happened**: A test was added to verify that `OperationalError` (e.g., database connection refused) propagates up from the `cli_main_indirectmanagednodeaudit` collector, in contrast to `ProgrammingError` (e.g., table doesn't exist) which is caught and logged with a fallback message. The existing collector error handling catches `ProgrammingError` specifically (for AAP 2.4 where the `main_indirectmanagednodeaudit` table doesn't exist) but intentionally lets `OperationalError` bubble up since it indicates a fundamental infrastructure problem that should not be silently swallowed.
- **Insight**: When catching database errors for graceful degradation (missing tables), use specific exception types (`ProgrammingError`) rather than broad catches (`DatabaseError`, `Exception`) -- infrastructure errors like `OperationalError` (connection refused, timeout) should propagate so they surface in monitoring and trigger alerts.

### Performance test tooling extended with indirect managed node audit data
- **Repo**: ansible/metrics-utility
- **Commits**: 0b1bf3e (#478)
- **What happened**: The performance testing tooling (`tools/anonymized_db_perf_data/`) was extended to include indirect managed node audit data. Changes: (1) `helpers.py` gained `create_indirect_managed_node_audits()` which distributes records across given jobs (cycling `job_ids[i % len(job_ids)]` and `host_ids[i % len(host_ids)]`), generates FQDN-style host names with optional unique suffix for collision avoidance, and batch-inserts in groups of 500. `delete_all()` was updated to delete indirect audit records first (before job events, respecting FK constraints). (2) `fill_perf_db_data()` gained an `--indirect-count` CLI argument (default 100) and now collects `job_id` return values from `fill_job()` to pass to the audit record creator. (3) `rollup_performance_test.py` was updated to include `main_indirectmanagednodeaudit` as a collector in performance measurements and include indirect node counts in the results report. (4) The `main_indirectmanagednodeaudit` collector's docstring was updated to clarify that `since`/`until` are optional (omitting them returns all records for a full-table snapshot), and tests were updated to call the collector without date params by default, adding a separate test for the date-filtered path.
- **Insight**: When adding a new collector to the performance test framework, update all three layers: data generation (helpers), orchestration (fill_perf_db_data with CLI args), and measurement (rollup_performance_test) -- missing any layer means the new collector won't be exercised at scale.

### cleanup_glob applied to remaining outlier test files
- **Repo**: ansible/metrics-utility
- **Commits**: 48a5846 (#450)
...
### Tests and perf tools switched/parametrized to the `unified_jobs_dashboard` collector
- **Repo**: ansible/metrics-utility
- **Commits**: 8bc7c80 (#538)
- **What happened**: With `unified_jobs_dashboard` now the collector the metrics service uses (it extends `unified_jobs` with project, launched_by, labels, and num_hosts fields), tests and tooling were updated to cover it. A dedicated `test_unified_jobs_dashboard_command` was added with its own expected CSV rows and skip-id columns. The anonymized-rollup helper `compute_anonymized_rollup()` gained a `unified_jobs_collector` parameter defaulting to `unified_jobs_dashboard`, and `test_from_gather_to_json` / `test_json` were `@pytest.mark.parametrize`d over `[unified_jobs, unified_jobs_dashboard]` so both the CLI and metrics-service collectors run through the full gather->rollup->JSON path. The perf tools (`rollup_performance_test.py`, `test_collectors_performance.py`) added the dashboard collector and labeled the two as "unified_jobs (CLI)" vs "unified_jobs_dashboard (metrics service)".
- **Insight**: When one collector variant supersedes another as the production path, parametrize the shared integration tests over both (with a swappable collector parameter defaulting to the new one) rather than duplicating whole test files -- and label the CLI vs metrics-service variants explicitly in perf tooling to keep the distinction clear.
...
### Segment tests move from SDK mocking to `requests.post` mocking, mock server learns gzip
- **Repo**: ansible/metrics-utility
- **Commits**: 5b28769 (#574)
- **What happened**: With the Segment SDK gone, `test_storage_segment.py` was rewritten around `@patch('requests.post')` (~28 tests) and now asserts on the wire format directly: that the body is gzip-compressed with `Content-Encoding: gzip`, that gzip can be disabled, that multiple chunks arrive in a single batch, that oversized batches split, that an indivisible oversized event is rejected *before* any POST, that HTTP errors propagate, that `message_id` in `segment_meta` produces stable IDs across retries, and that SDK-compatible value coercion (datetime, `Decimal`, `Enum`, nested metadata) still holds. Host validation gets its own cluster of tests -- remote HTTP rejected, loopback allowed without opt-in, single-label internal host allowed only with `allow_insecure_host=True`. The Go mock server (`tools/mock-segment-server/main.go`) gained transparent gzip decoding of request bodies and a new `content_encoding` field on each captured record, so tests can assert compression was actually used. The end-to-end `test_from_gather_to_json.py` assertion changed shape: it no longer expects one POST per chunk (the old `sync_mode` contract) but flattens `batch` arrays across all captured requests and asserts `len(received_events) == len(chunks)`, plus `path == '/v1/batch'` and `content_encoding == 'gzip'` per request. It also had to pass `allow_insecure_host=True` because the compose hostname `mock-segment` is a single-label HTTP host.
- **Insight**: Swapping a transport changes what the integration test can assert about *shape* -- "one request per chunk" was an artifact of `sync_mode`, so the assertion had to be rewritten to count events across batches rather than count requests. Teaching the mock server to record `Content-Encoding` (and decode it) is what makes "we gzip by default" testable rather than assumed; and a new security guard like `allow_insecure_host` immediately shows up as required plumbing in every existing test that pointed at a non-loopback mock.

### Cron classification pinned by a 3x2x3 parametrized query test
- **Repo**: ansible/metrics-service
- **Commits**: ac7f79e (#435)
- **What happened**: `test_scheduling_queries_classify_cron_expressions` stacks three `@pytest.mark.parametrize` decorators -- `cron_expression` in `(None, "", "0 * * * *")`, `status` in `("pending", "completed")`, `delay` in `(None, -1, 1)` -- creates one Task per combination and asserts membership in `ready_to_run()`, `immediate_tasks()` and `recurring_tasks()` against an expectation *derived* in the test body (`one_shot = cron_expression in (None, "")`) rather than hard-coded per case. The same PR flipped `test_cleanup_old_tasks`'s deliberately-wrong assertion (which had documented the empty-cron bug behind a FIXME) and added a task exercising the `modified`-date fallback path.
- **Insight**: Where a bug is "one of N representations was missed", parametrizing over all N representations and computing the expectation from the same rule the code should follow catches the next omission too -- and it is far shorter than N hand-written cases.

### TemplateMetadata concurrent-insert race covered with real threads and barriers against PostgreSQL
- **Repo**: ansible/metrics-service
- **Commits**: ac7f79e (#435)
...
### Test event collection tooling for local AWX environments
- **Repo**: ansible/metrics-utility
- **Commits**: 9918c61 (#501)
- **What happened**: A comprehensive test tooling suite was added under `tools/test_playbook_awx/` for exercising event collection against a local AWX instance. The suite includes: (1) `rich_playbook.yml` -- a 287-line Ansible playbook covering diverse event types (module calls across multiple collections: `ansible.network`, `redhat.rhel_system_roles`, custom roles), loops, conditionals, block/rescue, async tasks, and multi-host patterns. (2) `run_rich_playbook.py` -- uploads the playbook and collection artifacts to AWX, creates a project/inventory/job template, and launches it. (3) `collect_events.py` -- runs the service collectors (`unified_jobs`, `job_host_summary_service`, `main_jobevent_service`) against the AWX DB and dumps results for inspection. (4) `clear_events.py` -- cleans up job data from the AWX DB for fresh test runs. (5) `run_all_on_awx_db.py` -- orchestrates the full cycle: clear -> run playbook -> wait -> collect. Mock Ansible collection structures (galaxy.yml + role tasks) are included for `ansible.network` and `redhat.rhel_system_roles`.
- **Insight**: Testing event collection end-to-end requires a playbook that generates diverse event types (different modules, collections, roles, async, loops) because the rollup pipeline aggregates by module/collection/role -- a simple single-task playbook would only exercise one code path. The tooling runs directly against a local AWX database rather than mocked data, catching SQL query issues that mocks miss.
