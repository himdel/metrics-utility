...
### initial_resource_sync given retry config to match gateway populate_service_id cycle
- **Repo**: ansible/metrics-service
- **Commits**: 44e30ac (#365)
...
- **Repo**: ansible/metrics-service
- **Commits**: 3620e96 (#360)
- **What happened**: `TaskViewSet.perform_update()` originally protected only `function_name`, `is_system_task`, and `cron_expression` on system tasks. Three additional fields were added to the protected set: `name`, `status`, and `task_data`. A subtlety: `status` is a `read_only` field in `TaskSerializer`, so it never appears in `serializer.validated_data`. The guard was extended to also inspect `self.request.data` (raw payload), so a PATCH containing `status` is rejected with 403 rather than silently accepted as a no-op 200. The check uses set intersection: `attempted_protected = protected_fields & (set(validated_data.keys()) | set(request.data.keys()))`.
- **Insight**: When protecting fields on system resources, check both `serializer.validated_data` (writable fields the serializer accepted) and `request.data` (raw payload including read-only fields the serializer silently strips). Without checking `request.data`, a client could send `{"status": "completed"}` and get a misleading 200 OK even though the field was not actually changed -- the correct response is 403 to signal the field is protected.

### Task state machine formally documented with Mermaid diagram and invariants
- **Repo**: ansible/metrics-service
- **Commits**: c9553b73 (#356)
- **What happened**: A `docs/task-state-machine.md` was added (296 lines) documenting the complete task state machine: five states (pending, running, completed, failed, cancelled), all transitions, and recovery handlers. Key design decisions codified: (1) Three task types (immediate, scheduled, recurring) differ only in how they enter `pending` -- once pending, all follow the same state machine. (2) Recurring tasks are templates that never run themselves; they spawn one-shot children on each cron fire. The template stays `pending` forever. (3) `_fail_stuck_tasks()`, `_retry_failed_tasks()`, and `_cleanup_stale_advisory_locks()` are the three distinct periodic-sync recovery handlers. (4) `update_task_status(status="running")` raises ValueError -- only `_claim_task()` transitions to running (atomically with attempt increment). (5) `retry()` is the sole entry point for re-queueing a failed task; it uses `queryset.filter(status="failed").update(...)` for atomic conditional state reset. (6) Retried tasks become scheduled tasks (scheduled_time = now + backoff delay).
- **Insight**: Documenting the state machine as a formal diagram with enumerated invariants transforms implicit knowledge (scattered across `tasks_system.py`, `cron_scheduler.py`, `models.py`) into a reviewable artifact. The key invariants -- "only `_claim_task` transitions to running", "recurring templates never leave pending", "retry() is the sole re-queue path" -- are the kind of constraints that code review alone cannot enforce consistently. Issue: AAP-75761.

### Segment send task gets both scheduler-side and sender-side ANONYMIZED_DATA_COLLECTION guards (defense-in-depth)
- **Repo**: ansible/metrics-service
- **Commits**: fb89eb7d (#399)
- **What happened**: Disabling `ANONYMIZED_DATA_COLLECTION` stopped the recurring `daily_anonymize_and_prepare` cron, but the one-shot `send_anonymized_to_segment` Task that `daily_anonymize_and_prepare` creates (scheduled at now+jitter, see #183) carried no enablement metadata — so a send task created before opt-out could still transmit to Segment. The fix adds two guards: (1) `daily_anonymize_and_prepare` now tags the dynamically created send task with `task_data={"payload_id": ..., "_feature_flag": "ANONYMIZED_DATA_COLLECTION"}`, so the scheduler's `_task_feature_flag_enabled()` check (from #199) skips it when the flag is off; (2) `send_anonymized_to_segment()` gained a runtime guard at the top that calls `get_feature_enabled_from_db("ANONYMIZED_DATA_COLLECTION")` and, when disabled, logs a "skipped" execution and returns a success result with zero counts — before touching any payloads. Tests that exercise send behavior now must explicitly override the flag on because test settings disable anonymized collection by default. Issue: AAP-87800.
- **Insight**: An opt-out flag must be enforced at every place a gated action can originate. Gating the recurring parent cron is not enough when it spawns one-shot child tasks that outlive the flag-check moment — tag the child with `_feature_flag` so the scheduler respects it, AND re-check the flag inside the sender at execution time. The runtime guard returns success-with-zero (not an error) so a legitimate opt-out doesn't churn the retry machinery.
