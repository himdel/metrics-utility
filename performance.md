...
### Feature flags resolved once per scheduler sync pass, still per-fire at execution time
- **Repo**: ansible/metrics-service
- **Commits**: 5cc096f (#427)
- **What happened**: `UnifiedTaskScheduler._task_feature_flag_enabled(task)` called `get_feature_enabled_from_db()` for every task carrying a `_feature_flag` in `task_data`, so a startup sync or a 30-second periodic sync resolved the same handful of flags once per task -- one DB round trip each. The helper gained an optional `feature_flags: dict[str, bool] | None = None` parameter used as a memo; `_sync_database_tasks()` and `_periodic_database_sync()` each create a **fresh dict per pass** and thread it through the immediate / scheduled / recurring loops. Passing `None` (the default, and what the execution path does) keeps the uncached behaviour, so a flag toggled between fires still takes effect on the next fire.
- **Insight**: Scope a cache to the loop that benefits from it by passing it in, rather than memoizing the resolver itself -- the same function then serves the "read it once per pass" caller and the "must be fresh every time" caller, and the lifetime of the cache is obvious from where the dict is constructed.

### Leaderboard endpoint bounded to a fixed handful of queries regardless of window volume
- **Repo**: ansible/metrics-service
- **Commits**: 7734e5c (#422)
- **What happened**: The 30-day leaderboard aggregate is deliberately budgeted: one `GROUP BY (day, organization)` feeding counts, both streak series and the org leaderboard in memory; one `GROUP BY template_id` plus one latest-name lookup for the featured template; one `GROUP BY launched_by_id` with three aggregates covering all three activity levels; one `DISTINCT ON (launched_by_id)` username lookup restricted to the <= 30 ids that actually surface in a top 10 (an empty `visible_ids` makes the `__in` a no-op with no query at all); and, for the caller only, a few bounded aggregates plus a single ordered status-column scan for the consecutive-success streak. Distinct active days are counted with `TruncDate(...).distinct()`, which is bounded to 30 rows, rather than by materialising the user's run history in Python -- an explicit comment contrasts that with `total_runs`, which can be arbitrarily large.
- **Insight**: An aggregate endpoint stays cheap when the query count is a property of the *response shape* (how many groupings it reports) instead of the data volume -- do the GROUP BYs in the database, roll them up in Python, and fetch display names only for the rows that will actually be rendered.
