...
- **Insight**: When a third-party SDK silently drops data on oversized batches and provides no reliable way to estimate true per-event overhead, sync_mode (one HTTP request per event) is the only reliable approach. **Supersedes** the `use_bulk` option from #249 and extends the chunking fixes from #281. **Itself superseded by 5b28769 (#574)**, which dropped the SDK entirely for direct HTTP `/v1/batch` sends -- and found that Segment's raw HTTP API accepts gzip fine, contrary to the SDK experience above.
...
- **Repo**: ansible/metrics-utility
- **Commits**: daefa84 (#480)
- **What happened**: The `EventModulesAnonymizedRollup` was fundamentally reshaped from inferred task-outcome classification to direct event-type counting. The old approach collapsed events per (job, host, task_uuid, module) to infer task outcomes (success, failed with retries, skipped, ignored), which was unreliable due to loops (both task-level and item-level events fire), block/rescue (extra events), and retries (runner_retry events). The new approach maps 1-to-1 to Ansible event types: `runner_on_ok_total`, `runner_on_failed_total`, `runner_on_unreachable_total`, `runner_on_async_ok_total`, `runner_on_async_failed_total`, `runner_item_on_ok_total`, `runner_item_on_failed_total`, `runner_retry_total`. The `ignore_errors_total` is tracked as a combined counter (not per-event) because ansible-core does not expose ignore_errors on runner_on_async_failed events. Event type constants were extracted to a shared `metrics_utility/event_types.py` (`RUNNER_EVENTS` frozenset) used by both the collector SQL and the rollup Python code. The SQL collector now extracts `ignore_errors` from two JSON paths (top-level `event_data.ignore_errors` for runner_on_failed, `event_data.res._ansible_ignore_errors` for runner_item_on_failed). `playbook_on_stats` events were removed from collection (high volume, low unique value). `event_data_length` (via `octet_length(e.event_data)`) was added for performance modeling.
- **Insight**: Direct event-type counting is more robust than inferred task outcomes because it eliminates the classification logic that was fragile across Ansible's execution model. The shared `event_types.py` module ensures the collector SQL and rollup Python always agree on which events to collect and count. The `ignore_errors` combined counter is a deliberate trade-off: per-event splitting would silently undercount ignored async failures.

### User removed from RBAC model registry to prevent a shared.user DABContentType breaking gateway
- **Repo**: ansible/metrics-service
- **Commits**: 43a90ab (#401)
- **What happened**: `core.User` was listed in `ANSIBLE_BASE_RBAC_MODEL_REGISTRY` (alongside Organization and Team) while User is also registered in the resource registry as a SharedResource. That combination caused DAB's `post_migrate` hook (`create_DAB_contenttypes()`) to create a `DABContentType` with `service="shared"`, `model="user"`, `app_label="core"`. Gateway's `migrate_service_data` imported that record, but gateway has no `core` app, so `DABContentType.model_class()` raised `LookupError` and returned a 500 on `/api/gateway/v1/role_definitions/`. Fix: removed `"core.User"` from the registry and added migration `0002_remove_shared_user_content_type` that deletes the stale `DABContentType(service="shared", model="user")` row (reverse = noop). User is a shared resource synced from gateway; its roles arrive via JWT sync, not local RBAC assignments, so it needs no local permission tracking.
- **Insight**: A resource that is synced from the gateway as a SharedResource (like User) must NOT also be in `ANSIBLE_BASE_RBAC_MODEL_REGISTRY` — doing so creates a `shared.<model>` DABContentType in the `core` app that the gateway can't resolve (no matching app_label), crashing the cross-service role_definitions endpoint; only locally-owned models (Organization, Team) belong in the RBAC registry.

### `docs/architecture.md` written as the single end-to-end map of the two interfaces
- **Repo**: ansible/metrics-utility
- **Commits**: ac2d48b (#562)
- **What happened**: A 204-line `docs/architecture.md` was added, documenting the repo as two related interfaces (the management commands providing end-to-end gather/report workflows, and the `metrics_utility.library` package providing reusable primitives) with Mermaid diagrams for the main data path (Controller DB / Prometheus -> collectors -> packages -> storage -> extractors -> dataframe engines -> dedup -> report builders -> XLSX), the anonymized path (raw -> prepare -> merge -> daily base -> combine -> anonymize -> split -> Segment), the gather sequence, and the `build_report` factory chain. It also writes down the operational contracts that are invisible when reading one module: `since` inclusive / `until` exclusive interval boundaries, `METRICS_UTILITY_MAX_GATHER_PERIOD_DAYS` (default 28), checkpoints advancing only for successfully gathered-and-shipped collections, dry runs neither shipping nor persisting checkpoints, and reports being skipped unless `--force`. The extension guide states the rule that new implementations go behind the relevant factory (package / extractor / dataframe / dedup / report / report-saver) rather than branching inside the management command, and that library collectors stay environment-independent. The README, CLI, environment, partition, and library docs were refreshed alongside it.
- **Insight**: The valuable part of an architecture doc is not the module list but the cross-cutting contracts -- interval boundary semantics, checkpoint-advance rules, and "add behind the factory, not in the command" -- which are exactly what a reader cannot infer from any single file.

### Library and anonymized_rollups pruned to the metrics-service consumption surface
- **Repo**: ansible/metrics-utility
- **Commits**: fd6626d (#540)
- **What happened**: With metrics-service established as the only external consumer of `metrics_utility.library` and `metrics_utility.anonymized_rollups`, both packages were trimmed to what it actually imports (-4015 lines). Removed from the library: the whole parallel reporting pipeline (`dataframes/` with `BaseDataframe`/`BaseTraditional`/the five dataframe classes, `reports.py`, `extractors.py`, `package.py`, `instants.py`, `utils.py` with `tempdir`/`last_gather`/`save_last_gather`, `csv_utils.py`, `debug.py`), the non-Segment storage backends (`StorageS3`, `StorageCRC`, `StorageCRCMutual`, `StorageDirectory`), `dashboard/filter_options`, and the `workers/` example scripts (which referenced APIs that no longer existed). `candlepin` moved out of the library to `metrics_utility/candlepin/` as CLI-only. What stayed: every collector (even ones metrics-service does not call yet), the output adapters, `CsvFileSplitter`, `StorageSegment`, and `lock`. `anonymized_rollups/__init__` dropped `BaseAnonymizedRollup`, `AnonymizedPayload`, `anonymize_data`, `create_anonymized_object`, `flatten_json_report`, and `sanitize_json` from `__all__`, keeping the rollup classes plus `anonymize_rollups`. The guiding rule stated in the PR: keep every collector, keep siblings of a used API, move CLI-only helpers out of the library, delete anything unused outright. The CLI keeps its own implementations of the removed pipeline under `automation_controller_billing/`.
- **Insight**: Once a shared library has exactly one external consumer, its public surface should be defined by what that consumer imports -- carrying a second, unused copy of the CLI's reporting pipeline inside the library was pure duplication, and (as #556 later showed) duplicated code drifts: a security fix applied to the library copy never reached the live CLI copy.

### AWX analytics collectors ported as a fifth `library/collectors/awx/` subpackage
- **Repo**: ansible/metrics-utility
- **Commits**: 88f8f70 (#573)
- **What happened**: 15 AWX analytics collectors were ported into a new `metrics_utility/library/collectors/awx/` package (`config`, `counts`, `cred_type_counts`, `events_table`, `host_metric_summary_monthly_table`, `host_metric_table`, `instance_info`, `inventory_counts`, `org_counts`, `projects_by_scm_type`, `query_info`, `unified_job_template_table`, `unified_jobs_table`, `workflow_job_node_table`, `workflow_job_template_node_table`), joining `controller/`, `dashboard/`, `others/` and `service/` in `collectors/__init__`. They follow the existing library contract: plain `@collector` functions with keyword-only `db=`/`since=`/`until=`/`output=` params and no env var reads. Snapshot collectors (`config`, `counts`, `host_metric_summary_monthly_table`) take no time range and return a `DictOutput`/full table dump; historical collectors (`events_table`, `unified_jobs_table`, `host_metric_table`) take a date window. The AWX ORM logic was translated to raw SQL -- e.g. `counts` replicates `.exclude(inventory_sources__source='controller').exclude(inventory__kind='constructed').values(name_lower=Lower('name')).distinct().count()` as `COUNT(DISTINCT LOWER(main_host.name))` with a `NOT EXISTS` clause, and `config` reads AWX settings straight out of `conf_setting` instead of `django.conf.settings`, falling back to `importlib.metadata.version('awx')` when no `main_instance` row has a version. The AWX `_service` name suffixes were dropped. Validated against an aap-dev AWX harness (15/15 collectors) plus 89 unit tests.
- **Insight**: Porting AWX's analytics collectors into the library means re-expressing Django ORM semantics as raw SQL and reading AWX settings from `conf_setting` rather than `django.conf.settings` -- the library's "no Controller environment, params only" rule forces the collectors to talk to the database, not to AWX's Python process.

### StorageSegment: Segment SDK dropped for direct gzipped HTTP `/v1/batch` sends
- **Repo**: ansible/metrics-utility
- **Commits**: 5b28769 (#574)
- **What happened**: `StorageSegment` no longer uses `segment-analytics-python` at all -- it builds Segment `track` envelopes itself and POSTs them with `requests` to `{host}/v1/batch`, gzip-compressed by default, HTTP Basic auth of `(write_key, '')`, 30s timeout. The `SEGMENT_AVAILABLE` flag now guards `import requests` instead of `import segment`. The send path is a pipeline of small helpers: `_serialize_value()` reimplements the SDK's JSON coercions (datetime/date -> isoformat, `Decimal` -> float, `Enum` -> its value, set/tuple -> list, recursively); `_canonicalize_timestamp()` produces one stable UTC-isoformat string so generated message IDs are reproducible; `_build_event()` derives `messageId = sha256(f'{base}_{chunk_number}')` where `base` is a caller-supplied `segment_meta['message_id']` or a deterministic `f'{artifact}:{event}:{anonymous_id}:{timestamp}'`; `_calculate_event_overhead()` measures a dummy empty-data event so `_split_into_chunks()` gets `REGULAR_MESSAGE_LIMIT (32KB) - overhead - EVENT_SIZE_SAFETY_MARGIN (128)`; `_validate_event_sizes()` rejects oversized events **before** any request goes out; `_split_into_batches()` packs events into bodies under `BATCH_SIZE_LIMIT` (475,000 bytes, deliberately under Segment's ~500KB) and raises rather than silently sending an oversized single event. `_send_batches()` validates the host: HTTPS always allowed, HTTP only for loopback (`localhost`/`127.0.0.1`/`::1`), and single-label internal hostnames (e.g. `mock-segment`) only with an explicit `allow_insecure_host=True`; dotted remote HTTP hosts are always rejected. `put()` gained an `anonymous_id=` kwarg (still a fresh UUID when omitted) and now lets `requests.RequestException` propagate. `_split_into_chunks()` also gained an `else` branch so non-list, non-dict top-level values become their own chunk instead of being dropped.
- **Insight**: Replacing an analytics SDK with hand-rolled HTTP is worth it when the SDK's worker-thread lifecycle is unsafe in the host runtime (dispatcherd tasks) and its batching is opaque -- direct sends make endpoint, compressed payload, status code and response body observable, let exceptions reach the task retry machinery instead of being swallowed, and put the batch-size ceiling under your control. The cost is that you must reimplement the SDK's value serialization exactly, or previously-accepted payloads start failing on `json.dumps`. **Supersedes** the `analytics.sync_mode = True` workaround from #383.

### Leaderboards computed from one day-by-org rollup over a window every metric shares
- **Repo**: ansible/metrics-service
- **Commits**: 7734e5c (#422), ac7f79e (#435)
- **What happened**: Every leaderboard number is derived from successful `JobData` runs in one shared window -- `window_start = today - 29 days` at 00:00 UTC up to `now`, with `finished__lte=now_utc` deliberately excluding future-dated runs (clock skew, bad data) so they cannot inflate counts while the day series, which only ever runs to today, ignores them. The backbone is a single `GROUP BY (TruncDate(finished), organization_id, organization_name)` with `.order_by()` first, because `JobData`'s model-level `-started` default ordering would otherwise leak into the GROUP BY. `job_runs`, `active_organizations`, the enterprise streak, the org streak and the whole organization leaderboard are in-memory rollups of those rows -- no per-metric queries. Only two things get their own queries and both are documented as to why: the featured template (folding `template_id` into the day/org group would multiply the row count) and the per-user `GROUP BY launched_by_id` that feeds all three activity levels via `Count("id")`, `Count("template_id", distinct=True)` and `Count(TruncDate("finished"), distinct=True)`. Achievements are thresholds over the same data: user-level `ignition`, `week_warrior` (7 consecutive active days), `month_warrior` (active every day in the window), `explorer` (5+ distinct templates), `centurion` (100+ runs), `reliable` (20+ back-to-back successes) and `accelerator` (more runs in the second half of the window than the first); org-level `sustained` (14 consecutive days), `rising` and `top_tier` (current rank <= 3, because rank history is not stored). `reliable` is the one metric that needs run *order*, so it is the one place that scans rows: an ordered, single-column `values_list("status")` of that one user's finished runs (successful / failed / error / canceled) counted in a single linear pass. Names are display-only throughout: templates and orgs are keyed by id so a mid-window rename stays one row, with the latest known name fetched separately for display. A standing `TODO` notes that everything org-scoped currently uses the *busiest* org rather than the caller's own, because membership data is not ingested yet -- the rank lookup is written generically so it keeps working when that changes.
- **Insight**: Derive every figure on a dashboard-style endpoint from one shared window and one shared aggregate, or the streak, the counts and the leaderboards silently disagree with each other; and when a metric needs ordering rather than aggregation, isolate it to a single-column scan instead of abandoning DB-side aggregation for the whole endpoint.

### Current user's AWX id resolved from their own most recent run, not from the local User pk
- **Repo**: ansible/metrics-service
- **Commits**: 7734e5c (#422)
- **What happened**: The leaderboard has to know which aggregated row belongs to the caller, but `JobData.launched_by_id` is an AWX user id while `request.user.pk` is a row in metrics-service's own `User` table (DAB's resource registry gives each service a separate User table, synced from gateway). Comparing them would silently match the wrong person. The viewset instead resolves the AWX id from data it already has: `successful_runs.filter(launched_by_username=request.user.get_username()).order_by("-finished").values_list("launched_by_id", flat=True).first()` -- the only link `JobData` offers back to a platform identity. A caller with no successful runs in the window therefore has `current_user_id = None`, which yields `current_user_rank: null` and an empty achievements list rather than an error.
- **Insight**: In a DAB multi-service deployment, a local `User` pk is not a platform identity -- any collected table that stores the platform's own ids must be joined through a shared natural key (here the username, taken off the most recent matching row), and the "no matching rows" case has to degrade to null rather than to a wrong match.

## Superseded / Semi-Obsolete

### StorageSegment via the `segment-analytics-python` SDK (incl. `sync_mode`)
- **Repo**: ansible/metrics-utility
- The whole SDK-based transport -- `analytics.write_key`/`analytics.debug` module globals, one `analytics.track()` per chunk, `analytics.flush()`, and the `analytics.sync_mode = True` fix from #383 that forced one blocking HTTP request per event to stop the SDK silently dropping oversized batches -- was removed in 5b28769 (#574) in favour of direct `requests.post` calls to `/v1/batch`. The `use_bulk`/`BULK_MESSAGE_LIMIT` option (#249, removed in #372) is doubly obsolete. Notably the earlier finding that "Segment silently rejects gzip-encoded bodies" did not hold for the raw HTTP API: #574 gzips every batch body by default and the mock server verifies `Content-Encoding: gzip` round-trips. See "StorageSegment: Segment SDK dropped for direct gzipped HTTP `/v1/batch` sends" above.

### Five storage implementations in `library/storage/`
- **Repo**: ansible/metrics-utility
- The library's `storage/` package (`StorageDirectory`, `StorageS3`, `StorageSegment`, `StorageCRC`, `StorageCRCMutual` behind a common `put`/`get`/`exists`/`remove`/`glob` API) was pruned in fd6626d (#540) to `StorageSegment` alone, since metrics-service only ever used the Segment backend. Artifact shipping is done by the CLI's own `automation_controller_billing/package/` classes (`PackageDirectory`, `PackageS3`, `PackageCRC`) selected by the package factory. See the "Library and anonymized_rollups pruned to the metrics-service consumption surface" entry above.

### Dataframe classes and the reporting pipeline inside `library/`
- **Repo**: ansible/metrics-utility
- `library/dataframes/`, `library/reports.py`, `library/extractors.py`, `library/package.py`, `library/instants.py`, `library/utils.py` and `library/csv_utils.py` were all removed in fd6626d (#540) -- metrics-service never imported them and the CLI kept its own copies under `automation_controller_billing/`. The loading/transform split those classes embodied is therefore only live in the CLI implementation now. The duplication had a real cost: the reDOS/correctness FQCN regex fix from #342 landed only on the library copy and had to be backported to the live CLI copy in #556.

### `workers/` example scripts
- **Repo**: ansible/metrics-utility
- The six `workers/*.py` example pipeline scripts (gather-crc, gather-full, rollup-hosts, report-ccspv2, cleanup-ccsp, report-renewal) were deleted in fd6626d (#540); they referenced library APIs that no longer existed and were removed from the codecov flag paths at the same time.

### Events rollup task-outcome inference (two-phase aggregation)
- **Repo**: ansible/metrics-utility
- The original event modules rollup (#215) used two-phase aggregation: first collapsing events to one row per (job, host, task_uuid, module) to get task-level outcomes, then aggregating to per-module statistics. This was replaced in daefa84 (#480) with direct event-type counting -- one counter per known Ansible event type, no inference. Old columns (task_ok_total, task_ok_with_retries_total, task_failed_total, task_unreachable_total, task_skipped_total, task_failed_and_ignored_total, unique_hosts_total) were removed. See architecture.md "Events rollup reshaped" entry.
...
### One-time resource sync of users and RBAC from gateway on init
- **Repo**: ansible/metrics-service
- **Commits**: 87784a9 (#320)
- **What happened**: Platform Auditor users added after install had no local RBAC data in the metrics DB. Their first request triggered a JWT claims callback to the gateway which failed because `service_id` was not registered, returning 403. The fix adds a `sync_resources_from_gateway` task that wraps DAB's `SyncExecutor` to pull all resource types (org -> team -> user) in order so orgs/teams exist before user role assignments reference them. The task is registered as a `cron=None` entry in `SYSTEM_TASKS_GROUP` (one-shot: runs once on init, not periodically), using the same pattern as `initial_dashboard_collection`. A periodic sync was initially added (every 15 min) but was removed because the gateway pushes role assignments in real-time via `POST /service-index/role-user-assignments/assign/`. The one-time backfill covers the upgrade path. Additionally, Organization and Team were flipped from `is_provider=True` (metrics.organization) to `is_provider=False` (shared.organization) with `OrganizationType`/`TeamType` serializers, matching the gateway's shared.* namespace -- the previous `is_provider=True` caused `ResourceType.DoesNotExist` during sync.
- **Insight**: Services that receive RBAC assignments via gateway push still need a one-time backfill on upgrade to seed data for users who existed before the sync was implemented. Using `cron=None` in task groups creates a one-shot task that runs after the service is fully started (vs inline sync during init, which blocks startup and may fail if the gateway isn't reachable yet). The `is_provider=True` vs `is_provider=False` distinction in DAB's resource registry determines whether the service owns the resource type (metrics.X) or consumes a shared one (shared.X) -- must match the gateway's namespace.
