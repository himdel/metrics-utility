...
### AWX schema dump refresh leaks metrics-utility's own helper functions into `latest.sql`
- **Repo**: ansible/metrics-utility
- **Commits**: e3df6b4 (#565)
- **What happened**: A routine refresh of `tools/docker/latest.sql` to AWX `devel` (df97f07, for awx#16458) produced a 38-line, insert-only diff containing **no AWX DDL at all** -- the entire delta was `CREATE FUNCTION public.metrics_utility_is_valid_json(text)` and `CREATE FUNCTION public.metrics_utility_parse_yaml_field(text, text)`, metrics-utility's *own* pl/pgsql helpers. They appear because the source database had metrics-utility run against it with write access, which creates them dynamically. These two functions already live in a separate `tools/docker/functions.sql`, loaded immediately after `latest.sql` (`init-1-schema.sql` then `init-2-functions.sql` in compose; a single `cat ... | psql` in `pytest.yml`), specifically because GitHub CI runs without write access. So both copies now run, the second `CREATE FUNCTION` errors with "already exists", and it is only survivable because neither loader uses `ON_ERROR_STOP`. Worse, the two copies have drifted: the dumped versions are *newer* (`is_valid_json` catches `invalid_text_representation` rather than bare `others`; `parse_yaml_field` `regexp_replace`-escapes the field name before building the regex) while `functions.sql` still holds the older, unescaped versions.
- **Insight**: A schema dump taken from a database the tool under test has already written to is not a clean upstream schema -- it captures your own runtime-created objects, silently duplicating a fixture you maintain by hand and letting the two copies drift without any test noticing. When a dump refresh diff contains only your own identifiers, that is the signal to re-take it against a virgin database (or to delete the now-redundant hand-maintained fixture), not to merge it.
- **Correction**: The provenance diagnosis above is wrong, and the real story is better -- see "AWX migration 0211: AWX now creates metrics-utility's pl/pgsql helpers" below. The functions in the refreshed `latest.sql` did **not** come from metrics-utility having written to the source database; they came from AWX's own migration `0211_create_metrics_utility_functions`, which is an ancestor of the dump ref `df97f07`. The dumped bodies match the AWX migration byte-for-byte (4-space indent, `WHEN invalid_text_representation`, `regexp_replace`-escaped field). So the dump is clean and correct, and it is `tools/docker/functions.sql` that is now the redundant, stale duplicate. The generic insight about self-polluted dumps still holds as a rule of thumb -- it just is not what happened here.

### AWX migration 0211: AWX now creates and owns metrics-utility's pl/pgsql helper functions
- **Repo**: ansible/awx
- **Commits**: b336400963 (#16458)
- **What happened**: Migration `0211_create_metrics_utility_functions.py` adds `CREATE OR REPLACE FUNCTION metrics_utility_parse_yaml_field(text, text)` and `metrics_utility_is_valid_json(text)` to the AWX controller database, plus `GRANT EXECUTE ... TO PUBLIC` on both. No tables, columns, indexes or constraints -- schema-level function objects only. It uses `dbawaremigrations.RunSQL` with `sqlite_sql=noop` / `sqlite_reverse_sql=noop` so AWX's SQLite-backed unit and functional tests are unaffected, and a `DROP FUNCTION IF EXISTS` reverse. The `GRANT ... TO PUBLIC` is deliberate: the metrics read-only user is customer-configurable (`ms_awx_readonly` is only the convention), so no specific role name can be hardcoded, and PUBLIC is the only grantee that covers every deployment. Depends on `0210_remove_host_last_job_fields`; the PR was renumbered twice (0206 -> 0210 -> 0211) as devel moved underneath it. Two review fixes landed inside the PR before merge: the `field` argument is now `regexp_replace`-escaped (`'[.+*?^${}()|\[\]\\]'` -> `'\\&'`) before being interpolated into the line regex, and `EXCEPTION WHEN others` was narrowed to `WHEN invalid_text_representation` (SQLSTATE 22P02) so only the expected JSON-cast failure is swallowed.
- **Insight**: None of our dependent tables change -- but the *ownership* of these two functions just moved upstream, and that is the whole point. metrics-utility no longer needs write access to the controller database to use them on AWX >= 0211: they are installed by `awx-manage migrate` and executable by any role. Read-only-connection collection (the metrics-service `ms_awx_readonly` path) is the motivating use case.

### The pl/pgsql helper duplication: AWX's copy is the good one, ours are two stale ones
- **Repo**: ansible/awx (consequence in ansible/metrics-utility)
- **Commits**: b336400963 (#16458); m-u side e3df6b4 (#565), open m-u PR #577
- **What happened**: After #16458 there are three copies of these functions in play and AWX's is the newest. (1) AWX migration 0211 -- escaped field, narrow `invalid_text_representation` catch. (2) m-u `tools/docker/functions.sql` -- the hand-maintained CI fixture, still the *old* bodies (unescaped `field`, bare `WHEN others`), and it uses plain `CREATE FUNCTION`, not `CREATE OR REPLACE`. (3) m-u `metrics_utility/library/collectors/util.py::_yaml_json_functions()`, called by `ensure_functions(db)` from the `main_host`, `job_host_summary` and `main_hostmetric` collectors -- also the *old* bodies, but with `CREATE OR REPLACE`. In CI the loader order is `latest.sql` (which now carries AWX's new versions, since the dump ref is post-0211) then `functions.sql`, whose `CREATE FUNCTION` fails with "already exists"; the failure is swallowed only because neither loader sets `ON_ERROR_STOP`, and by luck the surviving definitions are the correct new ones. At runtime the hazard runs the other way: against a real AWX >= 0211 database with write access, `ensure_functions()` issues `CREATE OR REPLACE` and **silently downgrades AWX's hardened functions back to the unescaped, `WHEN others` versions**. Open m-u PR #577 ("github CI - surface database import errors, drop duplicate sql functions") is the correct resolution for the CI half -- add `ON_ERROR_STOP` and delete the now-redundant `functions.sql`.
- **Insight**: When upstream adopts a helper you used to install yourself, your installer stops being a convenience and becomes a downgrade path -- `CREATE OR REPLACE` does not ask whether the existing definition is newer than yours. #16458 resolves the CI duplication (delete `functions.sql`, keep the dump) but *creates* a runtime one: `ensure_functions()` should become a version-aware no-op (or be dropped entirely, gated on the AWX migration level) rather than unconditionally overwriting. Note the collectors already work around missing functions for the read-only service path (`# ensure_functions writes to DB, cannot be used in service (readonly DB)`) -- on AWX >= 0211 that caveat is obsolete and the write path is the only remaining reason to call it.

### AWX #16639: custom RBAC role names with no legacy equivalent now return None (no schema change)
- **Repo**: ansible/awx
- **Commits**: c0e1e7e6cb (#16639)
- **What happened**: AAP-79229. `get_role_from_object_role()` in `models/rbac.py` inferred the legacy role field by `rd.name.split()` into exactly two or three words; a custom role definition with a multi-word name (`Test Custom 1`) raised `ValueError: too many values to unpack`, and a custom role whose permission matrix duplicates a managed role could be picked non-deterministically, giving HTTP 500/403 on resource creation. The branchy inline logic was split into `_legacy_role_name*` helpers guarded by `LEGACY_ROLE_NAME_PARTS = 2` / `LEGACY_SCOPED_ROLE_NAME_PARTS = 3` token-count checks; names matching no convention log a debug line and return `None`, and `apps.get_model()` lookups are wrapped in `try/except LookupError`. The two `ResourceAccessListElementSerializer` callers in `api/serializers.py` gained `if old_role is None: continue`. No migration file, no model field/table/index changes.
- **Insight**: Python-only RBAC translation fix -- no DDL, and DAB RBAC tables are not in our dependent tables list, so no collector is affected. Worth noting only because it touches `main/models/rbac.py`, which is in our migration-watch path but in this case carries no schema consequence.
