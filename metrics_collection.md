...
### Event collector gains job/row limits and partition-density-based job selection
- **Repo**: ansible/metrics-utility
- **Commits**: 66f0441 (#451)
- **What happened**: The `main_jobevent_service` collector was significantly refactored for performance and safety. Two new limits were added: `job_limit` (default 1,000) caps the number of jobs whose events are collected, and `row_limit` (default 200,000) caps the total event rows returned. A `_select_jobs_by_partition_density()` function was added that groups jobs by their `job_created` hour (matching PostgreSQL partition boundaries) and selects from the densest partitions first -- this ensures the event query scans as few partitions as possible. The existing hour-range-based WHERE clause building was extracted into reusable `_build_job_created_ranges()` and `_build_timestamp_where()` helper functions. A `_normalize_limit()` helper validates limit values (non-integer falls back to default, zero means no limit, negative falls back to default). When the job limit is reached, an info log directs operators to increase `METRICS_SERVICE_JOBEVENT_JOB_LIMIT`. The `_RELEVANT_EVENTS` list was also made explicit as a module constant.
- **Insight**: For large deployments with 100K+ jobs per hour, collecting events for ALL jobs is infeasible -- prioritizing dense partitions (where many jobs share the same `job_created` hour) minimizes partition scans while maximizing the number of represented jobs per partition accessed.

### Anonymization of custom items changed from masking to removal
- **Repo**: ansible/metrics-utility
- **Commits**: 66f0441 (#451)
- **What happened**: The anonymization strategy for custom (non-whitelisted) collections, modules, roles, and installed collections was changed from masking to removal. Previously, `_anonymize_custom_items()` replaced field values with the string `"Custom"` for items whose `collection_source` was `"Custom"`. The replacement functions (`_remove_custom_items`, `_remove_unknown_installed_collections`) now filter out these items entirely from the output lists. This means private module/collection/role names never appear in the outbound payload at all, not even as `"Custom"` placeholders. The `anonymize_data()` function was also simplified: instead of calling three separate `_anonymize_custom_items()` calls with different field lists, it loops over `('module_stats', 'collection_stats', 'role_stats')` and applies `_remove_custom_items` uniformly.
- **Insight**: Removing items with custom/unknown sources entirely (rather than masking with "Custom") is a stronger privacy guarantee -- it eliminates the possibility of inferring information from the count or position of "Custom" entries in the payload.

### Event rollup job-level metrics deduplicated per unique job_id
- **Repo**: ansible/metrics-utility
- **Commits**: 66f0441 (#451)
- **What happened**: The `EventModulesAnonymizedRollup._get_common_aggregation()` was refactored to prevent job-level metric inflation. Previously, job-level counters (jobs_total, jobs_successful_total, jobs_failed_total, duration totals, never-started total) were computed by simple aggregation over the task_summary DataFrame, which meant a job running the same module in N tasks would inflate every job-level counter by N. The fix: `jobs_successful_total` and `jobs_failed_total` now use `nunique()` on deduplicated job_ids filtered by `job_failed`, and duration/timing metrics use a `_unique_job_sum()` helper that sums column values only for the first occurrence of each job_id. The method signature was changed to accept `task_summary` as a parameter to access per-row columns for the closure-based aggregation functions.
- **Insight**: When aggregating job-level metrics from task-level data, simple `.sum()` overcounts if a job appears in multiple tasks -- use `nunique()` for counts and index-based deduplication for sums to get correct per-job values.

### Dashboard jobs collector excludes workflow child jobs alongside sync jobs
- **Repo**: ansible/metrics-utility
- **Commits**: 628461f (#525)
- **What happened**: The `get_where_clause()` function in `library/collectors/dashboard/queries.py` previously excluded only `launch_type = 'sync'` from dashboard job collection. Workflow child jobs (`launch_type = 'workflow'`) were included in the Successful/Failed totals but excluded from the Top 5 Users chart because `launched_by_id` is NULL for non-manual/relaunch jobs. This caused a persistent mismatch between dashboard totals (AAP-74848, AAP-85129). The fix changed `uj.launch_type != %s` to `uj.launch_type NOT IN (%s, %s)` and added `'workflow'` to the params list. Tests were updated to match the new parameterized query shape.
- **Insight**: When a SQL WHERE clause excludes certain record types, all exclusion criteria must be applied consistently -- if a record type has NULL in a field that another query uses for grouping (like `launched_by_id` for Top 5 Users), the totals will diverge. Dashboard collectors should only collect records that ALL dashboard views can fully process. See also the companion fix in [metrics_service.md](metrics_service.md#workflow-child-jobs-excluded-from-hourly-dashboard-sync-hook) (m-s #388) which applied the same exclusion to the independent hourly sync hook data path.

### Hourly dashboard sync hook also excludes workflow child jobs (companion to m-u #525)
- **Repo**: ansible/metrics-service
- **Commits**: f2822d33 (#388)
- **What happened**: The batch collector fix above (m-u #525) excluded `launch_type='workflow'` from the `get_where_clause()` SQL, but the hourly incremental sync hook (`_build_dashboard_sync_hook` in `collect_hourly_metrics.py`) is an independent data path with its own pandas filter. It only excluded `launch_type='sync'`, so workflow child jobs still entered `JobData` through the hourly path, causing the same totals mismatch. The fix changed `raw_data["launch_type"] != "sync"` to `~raw_data["launch_type"].isin(["sync", "workflow"])`.
- **Insight**: When two independent data paths (batch SQL collector and hourly pandas hook) write to the same table, both must apply identical exclusion criteria. Fixing one path without auditing the other creates a window where the unfixed path reintroduces the data it was supposed to exclude.

### host_metric SQL parameterized and keyset pagination fixed with multi-column comparison
- **Repo**: ansible/metrics-utility
- **Commits**: 1b48d16 (#530)
- **What happened**: The `ExtractorControllerDB.host_metric_query()` method had two issues: (1) the `since` timestamp and pagination marker were interpolated into the SQL string via f-strings, flagged as a SQL injection risk (AAP-85766); (2) the keyset pagination used `CONCAT(hostname, '___', COALESCE(host_id, 0)) > '{marker}'` string concatenation, which is fragile and prevents index usage. The fix parameterized all caller-influenced values (since, pagination marker hostname/host_id, and LIMIT) as query parameters via `%s` placeholders. The keyset pagination was rewritten to use proper multi-column comparison: `(hostname > %s OR (hostname = %s AND COALESCE(main_host.id, 0) > %s))`, matching the new `ORDER BY hostname ASC, COALESCE(main_host.id, 0) ASC`. The method signature changed from returning a query string to returning `(query, params)` tuple. The previously commented-out multi-column ORDER BY was activated, and the CONCAT-based ordering was removed.
- **Insight**: Multi-column keyset pagination must use expanded `(a > %s OR (a = %s AND b > %s))` comparison rather than CONCAT-based string comparison -- the concatenation approach produces incorrect ordering when values contain the separator character and prevents the database from using composite indexes. Always return `(query, params)` tuples from query-building functions to enforce parameterized execution.

### host_metric SQL moved into a `main_hostmetric` library collector with generic `sql_keyset()` pagination
- **Repo**: ansible/metrics-utility
- **Commits**: 27b8976 (#536)
- **What happened**: The host_metric query (and its custom pg helper functions) was extracted out of `ExtractorControllerDB` into a proper `main_hostmetric` library collector (`library/collectors/controller/main_hostmetric.py`), keeping the parameterized SQL and multi-column keyset pagination from #530. The `since`/`until` window now uses the shared `date_where()` helper (matching all other collectors); only the keyset marker and LIMIT remain bound `%s` params. Pagination was generalized into a new `sql_keyset(db, build_page, next_marker, page_size)` method on the output adapters: `DataframeOutput` loops keyset pages and concatenates them, while `CollectionOutput` streams the whole result set in one COPY (COPY streams server-side, so it ignores the marker/limit -- accepted only for interface parity). `ExtractorControllerDB` became a thin adapter that calls `main_hostmetric(...).gather()` for the Renewal Guidance report, and a new `main_hostmetric` gather collector (csv, daily_slicing) makes the same source data available to metrics-service. Two follow-up fixes in the same PR: (1) the gather collector was registered and added to `run-ccsp2-gather` but missing from `VALID_COLLECTORS`, so any gather including it failed validation -- it was added, plus a test asserting every registered optional collector is allowlisted; (2) `DataframeOutput.sql_keyset` fetched a trailing empty page whenever the row count was an exact multiple of page_size, and concatenating that all-object empty frame downgraded every column's dtype to object -- empty pages are now skipped before concat. Note `ensure_functions(db)` writes to the DB, so the collector cannot run against the read-only service DB.
- **Insight**: Owning the SQL, pg helper functions, and pagination in one library collector lets both the Renewal Guidance report (DataFrame output) and the metrics-service gather (COPY-to-CSV output) share it via the same `sql_keyset()` interface -- but concatenating a trailing empty pandas frame silently downgrades column dtypes, so empty pages must be skipped, and every registered optional collector must also be in `VALID_COLLECTORS` or gather validation rejects it.

### main_host collectors derive last_automation from main_jobhostsummary after AWX dropped the FK columns
- **Repo**: ansible/metrics-utility
- **Commits**: fb75800 (#567)
- **What happened**: AWX migration 0210 (#16529) dropped `main_host.last_job_id` and `main_host.last_job_host_summary_id`, so the `main_host` / `main_host_daily` collectors' `LEFT JOIN main_unifiedjob ON main_unifiedjob.id = main_host.last_job_id` no longer compiles against AWX 0210+ schemas. `_main_host_query()` now reaches `main_unifiedjob` through the latest job host summary per host: a `LEFT JOIN LATERAL (SELECT job_id FROM main_jobhostsummary WHERE host_id = main_host.id ORDER BY id DESC LIMIT 1) AS latest_job_host_summary ON TRUE`, then `LEFT JOIN main_unifiedjob ON main_unifiedjob.id = latest_job_host_summary.job_id`. The `tools/docker/latest.sql` fixture was refreshed to the post-0210 schema (both columns, their indexes, and the FK constraint removed), so the old query cannot silently survive in tests. `docs/collectors-and-partitions.md` was updated to list `main_jobhostsummary` as a read dependency of both collectors.
- **Insight**: `LEFT JOIN LATERAL ... ORDER BY id DESC LIMIT 1` is the portable "latest row per parent" replacement for a dropped denormalized FK, and it matches the `with_latest_summary_id()` pattern AWX itself moved to (backed by the `main_jobhostsumm_host_id_desc` index from migration 0206) -- refreshing the schema fixture in the same PR is what turns the upstream drop from a production surprise into a test failure.
...
### Events collector re-enabled with row and job limits
- **Repo**: ansible/metrics-service
- **Commits**: 8daf1af (#295), 7b16727 (#300)
- **What happened**: The `main_jobevent_service` hourly collector was previously disabled by default (`enabled: False` in task_groups) due to performance concerns. PR #295 re-enabled it (`enabled: True`), un-commented `EventModulesAnonymizedRollup` from the daily rollup processor, and added a `JOBEVENT_ROW_LIMIT` setting (initially 1M rows, configurable via `METRICS_SERVICE_JOBEVENT_ROW_LIMIT`). PR #300 then reduced the default row limit to 200,000 and added a `JOBEVENT_JOB_LIMIT` of 1,000 (max finished jobs processed per hourly window, keeping the SQL `IN` clause manageable). Both limits are injected into `collector_kwargs` only for the `main_jobevent_service` collector type. At ~700-900 bytes/row, 200K rows is ~140-180 MB in memory.
- **Insight**: When re-enabling a previously disabled collector, add configurable resource limits rather than running unbounded. Two dimensions of limiting (rows and jobs) provide independent controls: row limit caps memory usage, job limit caps SQL query complexity. The limits are type-checked at the collector call site (`if collector_type == "main_jobevent_service"`) rather than passed generically, keeping the interface clean for collectors that don't need them.

### Indirect managed nodes collector and anonymization integration
- **Repo**: ansible/metrics-service
- **Commits**: 64e50c6 (#274), 8d85e6f (#296), 69abb56 (#301), f73fd16 (#315), 7db1875 (#331), 0946897 (#332)
- **What happened**: Six PRs built out the indirect managed nodes collection pipeline. (1) #274 added the `indirect_managed_nodes` hourly collector entry (using `main_indirectmanagednodeaudit` from metrics-utility and `IndirectManagedNodesAnonymizedRollup`), scheduled at XX:25. (2) #296 integrated indirect node data into the daily anonymization pipeline. (3) #301 extracted the indirect node collector into its own `INDIRECT_NODE_COLLECTION_GROUP` task group with a separate `INDIRECT_NODE_COLLECTION` feature flag (default: False, customer opt-in -- later flipped to `True` by default at the AAP 2.7 GA in 53b0835 #413, see settings_and_configuration.md). (4) #315 fixed a critical bug: the task was configured with `"function": "collect_indirect_nodes"` which was never registered in `TASK_FUNCTIONS` -- changed to use `collect_hourly_metrics` with `collector_type` arg. (5) #331 moved the collector from hourly to snapshot pipeline (`collect_snapshot_metrics`) since the underlying table scan has no date filter, making hourly scheduling wasteful. (6) #332 moved the collector from snapshot to daily pipeline (`collect_daily_metrics`) with per-collector database routing (`"database": "awx"`) after the metrics-utility side was converted to `daily_slicing` filtered by `job.finished`. The rollup processor was moved from `hourly_rollup_processors` to `daily_rollup_processors` in the daily metrics rollup. Task schedule changed to `"55 1 * * *"` (daily at 1:55 AM).
- **Insight**: The six-PR arc shows the full lifecycle: hourly -> feature-gated group -> fix silent registration bug -> snapshot -> daily with DB routing. The daily pipeline's `_get_daily_collectors()` registry now supports per-collector `"database"` keys, defaulting to `"awx"`, which allows daily collectors to query different databases (metrics-service DB for `task_executions_service`, AWX DB for `indirect_managed_nodes`).

### Dashboard collection telemetry: self-observability for the collection pipeline
- **Repo**: ansible/metrics-service
- **Commits**: 5e0bd10 (#269)
- **What happened**: A new `DashboardTelemetry` model was added to store per-run performance telemetry for dashboard collection tasks: `task_name`, `collection_run_date`, `success`, `collection_duration_ms`, `rows_collected`, `sync_duration_ms`, `rows_synced`. Collection tasks (`sync_dashboard_job_records`, `sync_dashboard_host_summaries`, `collect_dashboard_reports_data`, `collect_dashboard_reports_initial_data`) now record telemetry via `DashboardTelemetry.record()` at the end of each run. A new REST endpoint `GET /api/v1/dashboard_reports/collection_telemetry/` (admin-only) returns the last 30 days of telemetry. Telemetry data is also forwarded to the anonymized Segment payload via `dashboard_collection_telemetry` in the daily rollup.
- **Insight**: Pipeline self-observability (recording how long collection/sync takes, how many rows were processed, whether it succeeded) is essential for diagnosing performance regressions and capacity planning. By storing telemetry in its own model rather than embedding it in task execution logs, it can be queried independently and exposed via API for monitoring dashboards.

### Incremental JobHostSummary sync via post_collect_hook
- **Repo**: ansible/metrics-service
- **Commits**: af738df (#285)
- **What happened**: The hourly dashboard sync (PR #210) preserved existing `JobHostSummary` records but never wrote new ones for jobs arriving after the initial backfill. The `total_number_of_unique_hosts` metric queries `JobHostSummary` directly with cross-job deduplication and cannot be computed from the per-job `num_hosts` count alone, causing it to be silently undercounted. The fix reuses the raw DataFrame from the existing `job_host_summary_service` hourly collector by adding a `post_collect_hook_factory` to its registry entry (mirroring the pattern used for `unified_jobs` in PR #210). The hook groups host summary rows by `job_remote_id`, coerces numpy types to native Python, chunks at 2000 records per task (keeping all records for a single job together), and schedules `sync_dashboard_host_summaries` Tasks. The task remaps `host_remote_id` -> `host_id`, looks up `JobData` by `job_id` (skipping non-terminal jobs), and calls `_sync_host_summaries` per job. Key review-driven improvements: batch DB queries instead of N+1 per-job lookups, stale chunk cleanup after update_or_create loop, record-count chunking instead of job-count chunking, and returning "error" status on partial failure to trigger retry.
- **Insight**: The `post_collect_hook_factory` pattern enables zero-additional-cost data reuse: the dashboard sync piggybacks on the existing hourly collector's raw DataFrame without issuing additional Controller DB queries. Record-count chunking (not job-count) ensures task payloads are bounded in size while keeping all records for a single job together (required by the stale-deletion logic in `_sync_host_summaries`).

### install_type added to anonymized Segment payload
- **Repo**: ansible/metrics-service
- **Commits**: 046e55e (#352)
- **What happened**: An `INSTALL_TYPE` Dynaconf setting was added to `apps/settings/defaults.py` (default: `"containerized"`, overridable via `METRICS_SERVICE_INSTALL_TYPE` env var). The value is included in the `summary_metadata` block of each daily anonymized Segment payload. The automation-metrics-operator injects `METRICS_SERVICE_INSTALL_TYPE=operator` via its env ConfigMap, so operator-managed deployments are distinguishable from containerized installer deployments in analytics.
- **Insight**: Using a Dynaconf setting with a sensible default (containerized) means only one deployment path (operator) needs extra configuration. This "default to the simpler path, override in the more managed path" pattern reduces configuration burden.
