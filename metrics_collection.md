...
- **What happened**: Three PRs built out the indirect managed nodes collection pipeline. (1) #274 added the `indirect_managed_nodes` hourly collector entry (using `main_indirectmanagednodeaudit` from metrics-utility and `IndirectManagedNodesAnonymizedRollup`), scheduled at XX:25. (2) #296 integrated indirect node data into the daily anonymization pipeline: added `IndirectManagedNodesAnonymizedRollup` to `hourly_rollup_processors` in `daily_metrics_rollup.py`, passed `indirect_managed_nodes_rollup` to `anonymize_rollups()` in `daily_anonymize_and_prepare.py`. The rollup's `prepare()` returns a deduplicated dict `{indirect_node_ids, indirect_nodes_total}` rather than a list of tagged records. (3) #301 extracted the indirect node collector into its own `INDIRECT_NODE_COLLECTION_GROUP` task group with a separate `INDIRECT_NODE_COLLECTION` feature flag (default: False, customer opt-in), using a dedicated `collect_indirect_nodes` function at XX:30. The cron scheduler's `_inject_dispatch_timestamps` was generalized from a single function check to `_PREVIOUS_HOUR_FUNCTIONS = {"collect_hourly_metrics", "collect_indirect_nodes"}`.
- **Insight**: The three-PR arc shows a deliberate architectural progression: first add the collector to the existing group to prove it works, then integrate into the anonymization pipeline, then extract into a separate feature-flag-gated group for independent opt-in control. Separating into its own task group with its own feature flag allows customers to enable indirect node collection without affecting other metrics collection, and vice versa.
